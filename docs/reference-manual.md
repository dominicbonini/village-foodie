...
Delta over V9.6 — the third batch of the same day. **Van filters on Schedule and Reports**, which surfaced that one column had been quietly doing two incompatible jobs. **A stale-props class found in the component that had been patching around it three times.** **URL persistence for the dashboard's event and tab.** **Server-confirmed writes**, and a definitive answer to a staleness question flagged twice. **The realtime publication turned out to contain two tables, not three — one subscription had never fired.** Settings-card conventions settled. **No new columns. Still no Stripe: company setup is on hold pending outside factors.**

- **🔴 `orders.van_id` IS A KDS ROUTING FIELD AND CANNOT CARRY ACCOUNTING MEANING.** Live on test-truck: **51 of 65 orders (78%) have `van_id = NULL` while their event is assigned to a van.** The walk-up path never sets it; the customer path sets it from the event row — which is precisely why it works as a walk-up discriminator, and precisely why it cannot be summed. A Reports filter keyed on it would have shown Van1's revenue as ~£300 of a real £1,447, and the sum across vans would not have reconciled to the truck total. **A column that is inconsistently populated ON PURPOSE cannot be given a second purpose that requires consistency.** Same family as `channel` vs `method`.

- **THE FILTER SELECTS EVENTS, NOT ORDERS.** The settled model: a van filter chooses which **events** a report covers, and every order belonging to a selected event is included whole. Nothing is apportioned. That retires the `van_id` question entirely and rests on `orders.event_id`, which is **0% null across all 65 orders** — stamped by `place_order_atomic` on every path, unlike `van_id`.

- **`eventsMap` WAS KEYED BY DATE AND MISLABELLING REAL ORDERS.** `if (!eventsMap[ev.event_date])` meant **first event on a date wins**, so on a two-event date every order from the second event was labelled with the **first** event's venue — in the on-screen history and in both CSV exports. Not "collapsed": wrong. Re-keyed to event id, with a date-keyed fallback retained so orders with a null `event_id` label exactly as before. **A fix, not a refactor**, and the same class as the recorded `id` (per-event display number) vs `order_key` confusion.

- **🔴 A STALE-PROPS CLASS, FOUND IN THE COMPONENT THAT HAD PATCHED AROUND IT THREE TIMES.** `AddOrderPanel` did `useState(todayEvent)` — a mount-time private copy — re-synced only when the event **id** changed. An override flipping on the **same** event changed no id, so the sync early-returned and the copy stayed stale; and the panel never remounts because it is hidden via CSS to preserve the basket. **Twenty lines below sat an effect whose only job was patching `status` through the same hole. This was the third field to fall through it.** Fixed by deriving `liveEvent` — identity from the operator's selection, values from the freshest available source — as a `const` on every render, with no effect and no dependency array. The `status` effect became genuinely redundant and was **removed**. ⚠️ Resolving against the parent prop alone would have been wrong: a Saturday phone pre-order taken on Friday would have got **Friday's** payment settings.

- **THE DASHBOARD'S EVENT AND TAB NOW PERSIST IN THE URL.** `?event=<uuid>` and `?tab=`, both falling through to the existing defaults on anything unrecognised — a foreign, deleted, cancelled, past or malformed value behaves exactly as no param at all. Ownership is validated by **membership in a server-scoped list**, never by trusting the id. ⚠️ **ONE `replaceState` effect keyed on both**, not two — two effects mutating the same URL in one commit work only by React's ordering and are silently fragile. `replaceState`, not `router.replace`: a pure address-bar rewrite with no navigation, no re-render and no refetch.

- **WRITES NOW RETURN WHAT THEY WROTE.** The two override handlers return the updated row and the client patches state from the response. ⚠️ Two deliberate departures from the obvious implementation, both avoiding recorded hazards: **`select('*')` rather than a named list** (a named select naming a missing column fails the whole statement with `42703` — the silent-empty-board class), and **no `.single()`** (it throws `PGRST116` on zero rows, turning a harmless no-op into a 500 for a write that simply matched nothing). **This is not the flip-back class:** those bugs set optimistic state *before* commit, leaving a window where the client held a value the server did not. Here the value arrives in a post-commit response — nothing is guessed, so nothing needs reverting.

- **THE TRUCK-LEVEL STALENESS SELF-HEALS — RESOLVED, NOT FLAGGED A THIRD TIME.** An eight-link chain, all in source and confirmed against the live publication: Manage saves → `update_truck` → writes the `trucks` row → the dashboard's `postgres_changes` subscription on that row → `reseedRef` → `fetchAll(pin, true)` → `forceSeed` opens the gate → `setTruck` runs. **`setTruck` was NOT moved**, and moving it would have been actively harmful: ungating it makes `truck` a new object every poll, re-running every effect that depends on it including the sound seed that protects Gusto's settings.

- **🔴 THE SUPABASE REALTIME PUBLICATION CONTAINS EXACTLY TWO TABLES: `orders` AND `trucks`.** A `truck_vans` subscription existed in the dashboard and **had never fired**, because the table is not published — and its stated main case had moved out from under it when `online_paused_until` relocated to `truck_events`. **Dead twice over, deleted.** Nothing was lost: the 60s poll calls the identical handler, and the acting device is already instant via `markPending`.

- **`truck_vans` MUST NEVER BE PUBLISHED.** `last_heartbeat_at` lives on that table and is UPDATEd **every 15 seconds per device**. `postgres_changes` filters by **row, not by column**, so there is no way to watch `paused_until` while ignoring `last_heartbeat_at` — publishing it would make every heartbeat from every van fire a full `/api/dashboard` refetch on every connected dashboard, scaling with devices × vans. Cross-device van pause therefore propagates in ≤60s, and that is **accepted, not a gap**. If instant propagation is ever wanted, the prerequisite is moving `last_heartbeat_at` off the table, not publishing it.

- **CASH WAS DE-NESTED FROM THE PAID STEP AND THEN RE-NESTED, ON A LAYOUT ARGUMENT.** The de-nesting reasoning — *"they answer different questions: WHEN do we take money vs HOW does it arrive"* — was true and was **the wrong test**. Two settings can answer different questions and still have one depend on the other, and structure should show the **dependency**. The dependency is real and driven by layout: with the paid step off, one tap means "paid AND collected", so splitting it into Cash/Card would make each button also collect — and the honest label `Cash & collected` does not fit a 72px label box at the 240px KDS column. **There is no honest way to render the split when the button also collects.**

- **SETTINGS-CARD CONVENTIONS SETTLED.** On the dashboard, each setting is its own card titled by the setting, with dependents nested beneath — so a group label and a grey inner panel were both removed as boxes within boxes with nothing to distinguish. In Manage, three groups share one Order settings card, so the sub-panels and their headings do real work and stay. **`SUBCARD_HEADING` is now a Manage-only token.** And: **per-event scope is a property of the SCREEN, not of each row** — Dashboard Settings is per-event, Manage is truck-wide, and repeating that on every row would read as a box of unrelated exceptions.

## V9.6 — 30 July 2026

Delta over V9.5 — the second batch of the same day. **A live outage on localhost whose cause had been latent for two passes, and whose symptom was a supported behaviour.** Nine dropped query errors found and made loud. **Per-event overrides for both payment settings**, including a reversal of a ruling made two passes earlier. **Sound configuration moved per-device** with Gusto's settings preserved. **The button vocabulary settled** — "Done" became "Collected", blue was retired, and the colour system now has one home. **Two columns added. Still no Stripe: company setup is the blocker, not code.**

- **🔴 THE SILENT-EMPTY-BOARD BUG — the worst symptom-to-cause distance yet.** `/api/dashboard`'s **events** query named `show_paid_step_override` and `takes_cash_override` before their migrations had been applied. PostgREST rejected the query with `42703` — and **the error was destructured away**, so `todayEvents` came back `null`, every `selectedEventId` branch required it non-null, the orders block never ran, and **the route returned HTTP 200 with `orders: []`**. No console error, no failed request, no server log. And an empty board is a **documented, supported state** (§5, "orders are strictly event-scoped"), so **the bug wore legitimate behaviour as a disguise.** It presented as the `Done` → `Collected` rename breaking a status filter; the rename was entirely innocent, and the bucketing compares DB status values only. `show_paid_step_override` had entered that select **two passes earlier and was already committed** — either column alone was sufficient, so the board had been broken since the truck-defaults pass and was only noticed later.

- **🔴 `select('*')` DEGRADES; A NAMED SELECT THROWS.** This asymmetry is the whole hazard. `total_minor`, `deal_savings` and `capacity_ack_at` were all added the same week and **none of them could have caused this**, because the orders and truck queries use `select('*')`. Only the events query names columns explicitly. So the risk is not "a new column" — it is **"a new column named in an explicit select"**. Enumerate those before deploying, every time.

- **NINE DROPPED QUERY ERRORS IN ONE ROUTE, NOT ONE.** The sweep of `/api/dashboard` found **five destructures covering nine queries** with no error capture: `truck_events`, `collection_times`, both `orders` queries, `menu_categories`, `menu_items_db`, `truck_vans`, `demo_sessions`. Only `trucks` and `order_payments` were already correct. All now log, and **each log names the CONSEQUENCE rather than just the failure** — "the board will render EMPTY", "prep secs read 0 and NOTHING counts toward capacity this poll", and so on. ⚠️ **`truck_vans` is the subtlest and runs every poll:** every consumer reads `van?.x ?? default`, so a failed query resolves to *"no capacity limit, no cooking step, no order-ready step"* — **a plausible configuration, not a visible fault.**

- **A dropped error plus a `?? default` consumer produces a plausible configuration, not an error.** This is the general form of the `truck_vans` finding and it also describes `paidStepFor`, which falls back to truck defaults when its event lookup fails — silently ignoring any per-event override. Harmless today only because both defaults are `false`; **the first operator to enable cash for one event would get card-only behaviour server-side with no signal anywhere.** Both now log.

- **ON LOCALHOST, THE WORKING TREE *IS* THE DEPLOY.** Code goes live the moment it is saved, ahead of any migration — so a "run the migration before deploying" classification is violated by default during development. That is how a correctly-classified deploy-coupled migration still broke the board. **Third time this week that migration-before-deploy has bitten, and each time it presented as something else.**

- **BOTH PAYMENT SETTINGS NOW HAVE PER-EVENT OVERRIDES.** `truck_events.show_paid_step_override` and `truck_events.takes_cash_override` — both nullable, **nullable-means-inherit, resolved at read time, NO bulk write and NO seeding at event creation.** ⚠️ This is a **deliberate divergence from `order_ready_override`**, which bulk-writes every existing event when the truck default changes and thereby wipes overrides the operator set on purpose. Fine for pause; wrong for "when do we take money" — an operator who set one event to take payment at order must not lose it because they changed their general default a week later. Because nothing is seeded, **an override never carries forward, which is also how it self-expires — no expiry mechanism was built or needed.**

- **`takes_cash` BECAME PER-EVENT, REVERSING A RULING FROM TWO PASSES EARLIER.** It had been settled as truck-level only, on the reasoning that "whether a truck accepts cash is a property of the business, not of a pitch". That reasoning was sound for the case in mind and **incomplete**: if the card terminal fails mid-service, the operator needs cash enabled **for tonight, from the dashboard**, without going into Manage. **"Is this a property of the business or the pitch" is the wrong question when equipment can fail.**

- **ONE RESOLUTION POINT: `lib/payments/paid-step.ts`.** `resolvePaidStep(truck, event)` is the only place either override is resolved, called by all six consumers including the two server-side reads. ⚠️ **`??` and NOT `||`** — an explicit override of `false` must be honoured, not fall through to a `true` default. `||` would treat `false` as "unset" and silently re-inherit; that is the one substitution that looks correct and is wrong, and there is a harness case asserting it for each setting.

- **SOUND CONFIGURATION IS NOW PER-DEVICE, VIA localStorage.** Following the `keep_screen_on` precedent and the V9.0 decision — sound is physical, so it belongs to the device, not the business. Key `hg_soundcfg_${token}`. **The mechanism that protected Gusto:** a device with nothing stored seeds from `trucks.sound_config` rather than from a hardcoded default, **and waits for the truck payload to arrive before seeding** — without that second guard, the async window between mount and payload would have silently reset their deliberately-configured `{"order_due":true,"new_orders":"all"}`. Read via a **lazy `useState` initializer, not a `useEffect`**: for a banner a one-frame flash is cosmetic, for sound the artefact would be **audible**. The write failure is logged three ways **and surfaced to the operator** as a toast — `keep_screen_on` wrapped the same write in a swallowed `try{}catch{}`, which is why "I turned it off and it came back" was possible. 🔴 **`trucks.sound_config` is RETAINED as the seed source** — retiring it requires that every device has loaded at least once since the deploy, or a device that has not would seed from nothing.

- **"DONE" BECAME "COLLECTED", AND THE BINDING CONSTRAINT WAS NOT THE OBVIOUS ONE.** The DB status is literally `collected`, and "Done" was ambiguous once the button split. `Mark paid & collected` (153px) **clips by 25px** — not in the button's own row, but in the **cooking-gate row**, where it shares space with the `⏳ Waiting` chip and has no horizontal padding of its own. Chosen: **`Paid & collected`**, at 116px **exactly the width of the string it replaces**, so it cannot regress any layout. Split states are `Mark paid` → `Collected`. **Copy only — every status, action and audit literal verified unchanged by count.**

- **THE COLOUR SYSTEM SETTLED, AND BLUE WAS RETIRED.** Blue was introduced for money actions and removed one pass later as foreign to a page whose vocabulary is orange, slate and green. Final: **orange = action (confirm, take money), green = ready (a kitchen state), slate = done.** Cash vs card is a **variant, not a kind**, so it is distinguished by **icon** (`💷` / `💳`) rather than by two near-identical oranges, which would read as a rendering mistake. All three colours live once in `lib/ui-tokens.ts` with their measured contrast ratios, joined by `SUBCARD_HEADING` after four inline copies of one class string appeared across two Settings screens.

- **TWO COLUMNS ADDED:** `truck_events.show_paid_step_override`, `truck_events.takes_cash_override`.

## V9.5 — 30 July 2026

Delta over V9.4 — the **29–30 July payments phase 1a/1b workstream**. Payments went from a design to a working, verified system: **the payment ledger, rollup, audit log and paid step are all built, deployed and confirmed against live data.** Three new silent-failure findings, one of which had been swallowing money-events since the day it shipped. A **documented-and-swept class reopened by the very next field added**, now fixed by inverting its failure mode. A **standing rule about scope** after this planning chat approved a UI change nobody asked for. **Three columns added.** **Status: still NO Stripe account, no Stripe code. Everything below is cash and own-PDQ, which is the point — phase 1 was designed to be provable without Stripe.**

- **🔴 THE IDEMPOTENCY KEY SWALLOWED EVERY CHARGE AFTER THE FIRST.** `collectIdempotencyKey` returned a **constant per order** — `collect:{order_key}` — so a second manual charge hit `23505`, was treated as a successful replay, and **recorded nothing, forever, with no error**. Pay £9.50 → edit up to £15 → tap "Mark £5.50 paid" → silence. The phase-1a migration header had *explicitly rejected* a composite unique index so that "£10 cash now, £5 later" would remain possible; the constant key forbade it anyway. **Design intent and implementation disagreed from day one and nobody noticed, because part-payment had never been exercised.**

- **NO DETERMINISTIC SERVER-DERIVED KEY CLOSES THE PROBLEM, AND THE DETECTOR MATTERS MORE THAN THE KEY.** Two candidate schemes were simulated across seven sequences. A **total**-based key fails on edit-down-refund-edit-up. A **balance**-based key fails on *"paid in full, customer comes back for another round"* — an ordinary hatch transaction, not an edge case. The adopted key is `collect:{order_key}:{paidBefore}:{balance}` (a state-transition key), which passes everything except the refund cycle. **That residue is not fixable with any deterministic key: if the key is a function of ledger state, any sequence returning the ledger to an earlier state and repeating the same transition must collide.** The complete answer is a client-minted per-tap key — `op_id` already exists in the outbox and is never transmitted. **So the real safety property is the DETECTOR:** a swallowed duplicate where the balance FELL is a genuine replay (silent, correct); a swallowed duplicate where the balance is UNCHANGED is a real collision (throws, surfaces as `paymentWarning` + a loud log + `ledger_failed: true`). It does not depend on the key scheme, and it would have caught this very bug on the first part-payment.

- **🔴 A NEW SILENT-FAILURE MEMBER — two different outcomes sharing one return shape.** `recordCollectionPayment` returned `{inserted: false, chargedMinor: 0}` for BOTH "already recorded" and "nothing outstanding". No caller branched on it. So the handler returned 200, the operator saw success, and **the audit log recorded a successful action that moved no money.** The information was technically present and structurally unusable.

- **🔴 THE PROJECTION-OMISSION CLASS WAS DOCUMENTED THREE TIMES, SWEPT AND CLOSED IN V9.0 — AND REOPENED BY THE NEXT FIELD ADDED.** `/api/dashboard` returned a hand-maintained include list; `show_paid_step` was not in it, so `data.truck.show_paid_step` was `undefined` and the toggle re-defaulted to `false` on every mount. **The V9.0 warning that "a newly-omitted field reopens the class" was sitting as a comment four lines above the omission.** A warning you must read and remember is not a control. Fixed by **spread-and-redact** — the row is spread and a redact list removes secrets — which **inverts the failure mode**: forgetting a field used to break a feature silently; now the worst case is a leaked field, which is visible and harmless. Verified against the live schema: 88 columns on `trucks`, all assessed, exactly four redacted (`dashboard_token`, `dashboard_pin`, `messenger_page_token`, `kds_pin`), zero false positives. **`takes_cash` was the first setting added after the fix and it reached the client with no map edit — the fix did what it was for.**
...
- **THE CAPACITY WARNING WAS CORRECT AND THE OPERATOR ACTION WAS LEGITIMATE.** Closing the 28 July investigation: window 18:15–18:20 genuinely held 4 pizzas against a ceiling of 2, and `van_id IS NULL` + `items[0].cartKey` proved two of the three contributing orders were **operator walk-ups**, which bypass all capacity gating by design. No defect. The display was the problem, and `❗` now marks strictly-over slots.

- **A STANDING RULE, AFTER BREAKING IT.** *Do not change UI the operator did not ask about.* An instruction to "make Ready match the toggles" could not be satisfied without changing the toggles; the coding chat flagged that prominently and then **resolved it itself in the direction that expanded scope**, and this planning chat approved it. Flagging is not a substitute for stopping. See §35.

- **THREE COLUMNS ADDED:** `trucks.show_paid_step`, `trucks.takes_cash`, `order_payments.method`.
...
Delta over V9.1 — the **27–28 July Android + demo/onboarding workstream**. One theme: the second native platform stopped being a theoretical port and started producing findings, most of which are **not Android-specific** — they are asymmetries that only became visible once a second platform existed. **Eleven new §35 invariants** (native throws pass through JS `try`/`catch`; one owner per safe-area inset; two APIs fronting one OS grant do not report it equally; a repo inside a synced folder gets conflict copies; a ported permission bypass without its routing branch widens access; an unordered first-row pick is non-deterministic; native tooling enforces filename/content rules the web never does). **Three corrections to existing entries.** **Two notification-preference migrations APPLIED**; a third **written and deliberately NOT run**. A new **§36 Android platform** section. **Status: the Android client half of push is VERIFIED end to end (a real FCM token on a real device); the SERVER half is not wired. No Android build has shipped and no store listing exists.**

- **§35 GAINED ELEVEN INVARIANTS — seven with a sweep status** per the standing rule. The two that cost the most time: **a JS `try`/`catch` around a Capacitor plugin call does not protect against a native throw** (there is no promise to reject — `PushNotificationsPlugin.register()` killed the process from inside a wrapped call), and **only one mechanism may own a safe-area inset** (Capacitor 8 pads the WebView on API 35+ *and* zeroes `env()`, so adding CSS padding recreates the two-band bug V8.7 removed on iOS).

- **THREE CORRECTIONS.** (1) V8.6 recorded `20260701_van_devices.sql` and `20260701_van_notification_prefs.sql` as **PENDING**; both are **APPLIED** (verified 27 July) — *recorded-as-pending, actually-applied*, the **inverse** of the §65 incident and the same class. (2) The "native token-only access defaults `userRole` to `'owner'`" note read as a general property of the native shell; it is **the MANAGE page's client-side `|| 'owner'` fallback**. `/api/dashboard` runs ONE role block for both cookie and Bearer, so a staff member on the native app correctly resolves to `'staff'`. (3) `lib/native/statusBar.ts` carried **three verified no-ops on modern Android** plus a hardcoded colour matching nothing in the brand — **a native helper written for one platform must be re-verified against the other, not assumed.**

- **NOTIFICATION PREFERENCES — `device_notification_prefs` APPLIED.** Per-device, per-type, replacing an accidental three-way split (per-VAN `van_notification_prefs`; the not-per-type `van_devices.notify_enabled`; device-local Capacitor Preferences the server cannot see and therefore cannot use to gate a push). **27 rows across 9 devices, verified.** `20260728_notification_prefs_retire_old_stores.sql` is **WRITTEN and DELIBERATELY NOT RUN** — it drops both old stores while `app/api/orders/submit/route.ts` still reads them. Both left **DORMANT** per the leave-dormant-then-sweep precedent. See §16.

- **§36 ANDROID PLATFORM created.** Capacitor 8 remote-URL shell shared with iOS; `android/**` inert on web and living on `main` per the V8.8 rule; **the UA marker is a per-platform config key, not a property of being native** — adding a platform means adding the marker or the V8.7 login loop returns; **Android push is FCM while iOS stays pure APNs, which makes `van_devices.platform` load-bearing**; and **iOS has never been capable of registering for push** — there is no `.entitlements` file and no `aps-environment` key.

- **⚠️ THE LANDING PAGE NOW STATES ANDROID IS AVAILABLE. IT IS NOT.** No build has shipped; no store listing exists. Recorded as a **CLAIM AHEAD OF THE PRODUCT** — the same class as the V9.0 *gate-enabled and advertised ≠ built* lesson. `/landing` is `noindex` but **not access-gated**, and is linked from the legal pages and the demo-return bounce. Deliberate forward commitment; a debt with a date, not a description of the software.

- **THE DEMO EVENT ROLL IS DELETED, AND AN ELAPSED DEMO NOW ENDS.** `rollDemoEventIfStale` shifted an elapsed demo forward on every dashboard load, and all three of its faults were confirmed against live data: it **wrote order slots directly, bypassing the seeder**, clamping overshoots onto the final slot (**15 orders on `23:59`**, breaching the per-slot mains ceiling the seeder guarantees); it preserved a compressed board's distribution **forever**; and it shifted **the visitor's own test order** — one placed at 23:40 reappeared at 09:45 the next morning. Replaced by an explicit **"Start a new service"** (`lib/demo-restart.ts`): a truck-wide wipe, a fresh window, a re-seed. ⚠️ **Truck-wide, not date-scoped** — `provisionDemoEvent(replaceExisting)` deletes by `event_date`, which would sail straight past yesterday's event. Full detail in the onboarding spec §10 Phase 2E.

- **SEEDED ORDER COUNT SCALES WITH WINDOW LENGTH — which also fixed the stride collapse.** The count was a constant regardless of window, so `stride = slots.length / budgets.length` hit **1** once a window fell to ≤~16 bookable slots: every slot got a budget and the taper flattened into a solid block. A **threshold effect, not gradual degradation** — the reason late-night demos looked broken while midday ones looked right. Scaling the target to bookable slots (`ORDERS_PER_SLOT ≈ 1.057`) keeps stride > 1 at every size. Before the fix a 5-slot window placed 14 of 37 and **silently dropped 23**.

- **SIGNUP COPY CENTRALISED into one variant-keyed object** (`upload` / `sample` / `saveOnly`) with a **deliberate upload/sample divergence** — someone who uploaded their own menu has an artifact worth naming, a sample visitor does not, so "save my menu" would be false for them. Sibling keys with identical shapes, so editing one shows the others unchanged in the same diff hunk. The `/api/dashboard` **`demo` block** supplies the variant and **retires the `?welcome=sample` param**, which survived exactly one navigation. **`canSetup` is now admin-aware** so the signup chain can be walked on production without opening signup publicly — not a client-trusted flag: `/api/signup` re-reads the session server-side and checks `operators.is_admin`.

- **`extraction_source` MIGRATION RECONCILED.** The column was written and read by code and present in production, but **in no migration file** — see §35's "a swallowed write makes a missing column indistinguishable from a working one". Now written as `add column if not exists`, a deliberate no-op against prod.

> **STATUS FOR THIS ENTRY.** **VERIFIED LIVE:** the scraper fixes — rule-detection loop, day-of-week learner disabled, failure visibility. **BUILT BUT UNVERIFIED:** all of the demo and signup work above, and the Android work. The Android client half of push is the one native thing verified end to end; its server half is not wired.

- **§27 GAINED FIVE ITEMS**, one of which is a blocker on another: the **iOS billing / App Store decision** (three options, all governed by an unverified question about whether an IAP obligation exists at all), the **notification honesty pass** (whose one-time `hg_notify_*` client migration MUST ship in the same release as the Settings card), the **FCM sender**, **Android hardware back**, and the **notification icon**.
...
Delta over V8.9 — the **16–17 July hardware-testing session** (first sustained run on a physical iPad). Fixed: an **app-lock HARD LOCKOUT** (a near-miss that would have shipped to an operator), the **wake-lock activation** (passive banner → real button), **keep-screen-on made per-device**, and an **offline slot-picker capacity gap**; plus the **`/api/dashboard` hand-picked-subset bug class SWEPT and CLOSED**. Two backlog items logged (sound → per-device, mobile sound control) and a standing lesson (**simulator-validated ≠ verified**). **Status: all of the above COMMITTED to `main` and pushed to origin (app-lock `847ee2d`, wake-lock `dd8d1f4`, keep-screen-on per-device `31ccbd1`), tsc-clean. ⚠️ On `main` + pushed means a production deploy may have already fired — Gusto is live tonight; confirm the deploy state before assuming these are/aren't in front of the live service. The two backlog items (sound → per-device, mobile sound control) are NOT built.** **⚠️ SEPARATE 17-JULY WORKSTREAM also in this section — the public `/landing` page + a plan-features GATE↔MARKETING reconciliation — lives on branch `landing-v32` and is UNPUSHED (so the "committed to `main`" status above covers the hardware fixes ONLY). See the PLAN/FEATURE MATRIX, `overflow-x` STICKY TRAP, `/landing`, PAY-AT-HATCH, CTA-CONTRAST, LANDING PAGE STATE, and COPY PRINCIPLE bullets.**

- **SUBSET-BUG CLASS — SWEPT + CLOSED (this is what V8.9 item 2 was missing).** Audited **every** truck-field read in `dashboard/page.tsx` and `kds/page.tsx` against the `/api/dashboard` truck map (`route.ts`). **Exactly TWO members ever existed: `sound_config` (V8.9) and `keep_screen_on` (this session) — both fixed.** Every other truck field read is present in the map; no destructuring / bracket-access reads hide any; the `truck` state is only ever populated from `data.truck`, so its shape **==** the map. **`/api/manage` is immune** (`getTruck` uses `select('*')`; its only hand-picked select is `id, name` for the truck-switcher, not a consumer object). **Do NOT re-audit this — the class is closed.** ⚠️ **But the RULE (V8.9 item 2) still stands for anything NEW:** a truck-level setting the dashboard or KDS reads must be hand-added to that map, which **still fails silently** if you forget — *closed ≠ can't-reopen*. A newly-omitted field reopens the class.
...
- **KEEP-SCREEN-ON → PER-DEVICE.** Was per-truck on web (`trucks.keep_screen_on`) but per-device on native (localStorage `hg_keepawake`) — already inconsistent — AND the web DB read was itself a **subset-bug victim** (`keep_screen_on` was never in the `/api/dashboard` map → `undefined ?? true` → **de-facto hardcoded ON on web: the operator's OFF never survived a reload**, so the KeepAwakePrompt banner reappeared for everyone every load). **Now per-device everywhere:** localStorage **`hg_keepawake_${token}`** (per-token, so two trucks on one iPad don't collide), read via a **LAZY `useState` initializer, SSR-guarded (`typeof window`)** — **NOT a `useEffect`.** *An effect runs after first paint and would flash the banner for one frame; the lazy initializer makes the pref known AT first paint — that's the whole subtlety.* **Unified web + native + KDS onto one path — KDS had NO per-device path before (it read the DB column even on native).** The `update_keep_screen_on` action is **removed**; `trucks.keep_screen_on` is **dormant** (drop it with the `Truck` interface fields in `manage/page.tsx` + `components/dashboard/types.ts` in a cleanup pass). **MODEL: keep-screen-on is PHYSICAL — this screen, in this van — so it's per-device, the same reasoning that moved sound (V8.9). A truck-wide setting forces the counter iPad and the owner's pocket phone to agree when they shouldn't.** Offline-protection is unaffected (`toggleOfflineProtection` reads runtime state, not the column). **THE BANNER RULE:** shows iff *(device pref ON, read synchronously)* AND *(lock not held)* — pref OFF → never (no flash); native → held from launch → never.

- **WAKE-LOCK ACTIVATION — properly fixed (SUPERSEDES the V8.9 item-3 gesture mechanics below).** Measured on tablet Safari: the "tap anywhere" banner FAILED, then the header toggle SUCCEEDED on the *same tap, seconds later*. **MECHANISM: `pointerdown` vs `click`, not the async gap.** The old banner had no handler; acquisition rode a **global one-shot listener on `pointerdown`/`touchend`/`keydown`**. `pointerdown` fires FIRST in the tap sequence and consumed the single shot — and **WebKit does NOT grant a user-activation for `wakeLock.request` on a bare `pointerdown`** (the *start* of a gesture, which could still become a scroll/drag); it grants it on the **completed** gesture — `click`, which the real `<button>` toggle fired. Chrome honours `pointerdown` too, so it read as Safari-only. **The async gap was NOT the cause** — the working toggle already `await`ed `getPlugin()` *before* the request and still succeeded, proving a microtask of an already-resolved promise doesn't spend Safari's transient activation — but it was closed anyway as hygiene. **THE FIX (`lib/native/keepAwake.ts` + dashboard/KDS):** (1) `KeepAwakePrompt` is now a **real `<button>`** reading **"Keep screen on"** whose `onClick` runs the **same** acquire path as the toggle (`onAcquire → applyKeepScreenOn(true) → keepAwake()`) — a `click` WebKit honours; (2) the **global gesture listener is removed** entirely (`armGestureAcquire` gone — an unreliable pointerdown request is exactly what failed); (3) `keepAwake()` reordered so `navigator.wakeLock.request('screen')` runs with **nothing awaited before it** — `isNativePlatform()` is a *synchronous* check and the only `await import()` lives on the native branch; (4) `keepAwakeOnGesture()` → **`prepareKeepAwake()`** (mount / pref-restore): native acquires immediately, web only sets intent + reflects state (no auto-request, no listener). Success/failure still flow through the existing `subscribeWakeState` binary toggle. ⚠️ **Verify on tablet SAFARI over https — the whole bug is Safari's stricter activation model; Chrome and native cannot prove it.**

- **⚠️ APP-LOCK HARD LOCKOUT — the near-miss that would have shipped to an operator (fix committed, branch `app-lock-fix`).** Enabling Touch ID **locked Dominic out of the app entirely** on the physical iPad. Two distinct defects, **both in the surrounding re-lock handler** — the biometric call itself was **textbook** (all four `@aparajita/capacitor-biometric-auth` v10 options correct; the resolve-on-success / throw-on-failure contract read right):
  - **THE LOOP.** On real iOS, presenting the Face/Touch ID prompt makes the app **resign-active**, and dismissing it — **INCLUDING ON SUCCESS** — makes it **become-active** → `appStateChange{isActive:true}` → the `onAppResume` handler `setLocked(true)` + `attempt()` → prompt again. Infinite. It was **`authenticate()`'s OWN lifecycle churn feeding the re-lock**, not a genuine background→foreground.
  - **NO ESCAPE HATCH (worse).** The in-prompt device-passcode fallback didn't help — entering it dismisses the prompt the same way success does, feeding the same loop. The only "off" switch lives in `OperatorDeviceConfig`, **inside the app**. Circular. **A biometric lock with no bypass = permanent lockout on a live-service device** (wet hands, a cut finger, a dead sensor = can't take orders at all).
  - **FIX — two parts.** (1) `authInProgress` ref, **CONSUME-ON-RESUME**: the become-active event *itself* clears the guard — **NOT a `setTimeout`** (a clock can be raced by a slow device; an event can't). A *later*, genuine foreground still re-locks, so the feature survives. (2) A **mandatory BACKUP PIN set at enable time** — you **cannot enable app-lock without one**, and that requirement is what actually closes the lockout. **PBKDF2-SHA256, 100k iterations + 16-byte salt** via Web Crypto (no bcrypt in a webview), stored hashed in native **Preferences**; auto-surfaced after 2 failed/cancelled biometric attempts.
  - **WHY A PIN, NOT THE ACCOUNT PASSWORD.** The password needs **network**. An operator whose prompt won't present at a rural venue with no signal would still be locked out — **exactly the scenario the app exists for**. A fallback that fails offline isn't a fallback. The PIN is a Preferences read + a local hash: **no network, no biometric, no plugin** — it mirrors iOS's own Face ID + passcode pairing.
  - **⚠️ RESIDUAL to watch:** consume-on-resume assumes the dismissal fires **exactly ONE** become-active. If a device fires two, the second re-locks and the loop returns → **needs a debounce on the consume** before this is trusted across more hardware.
  - **ESCAPE IF LOCKED OUT:** Safari Web Inspector → console → `localStorage.removeItem('hg_app_lock')` → reload. A **rebuild / reinstall-over-existing does NOT clear it** (same bundle id keeps its WKWebView data container); **only delete-then-reinstall** does.

- **⚠️ STANDING LESSON — SIMULATOR-VALIDATED ≠ VERIFIED.** The app-lock loop was **simulator-validated and passed**: the simulator's "Matching Face" does **not** reproduce the resign / become-active lifecycle a real Secure Enclave prompt causes. In one afternoon the physical iPad found **THREE** defects the simulator masked — the lockout, the offline slot-picker gap, and the wake-lock activation bug. The manual's **"hardware-gated, unverified"** warnings were pointing at **real defects, not hypotheticals**. **Anything touching a native prompt, the screen, or the network lifecycle is UNVERIFIED until it runs on hardware** — a simulator pass means "compiles," not "works."

- **OFFLINE SLOT PICKER — capacity lights missing offline (found on the device).** Offline, the operator saw slot **times** but **no capacity traffic-lights**: the picker fetches its own **`/api/slots`, which is NOT in the SW cache** (unlike `/api/dashboard`), and its `catch` silently **NULLed `capacityInputs`** → fell to a bare-time fallback. **NOT a deliberate honesty stance** — the day strip (`displaySlots`) already computes **advisory** offline capacity from the cached inputs with an explicit comment sanctioning it; the picker was simply never wired to that path (contrast the events `catch`, which explicitly preserves: "must not wipe the list"). **FIX:** extracted **`buildOfflineOccupancy` into `lib/slot-capacity.ts`** — **ONE shared fold** used by both strip and picker so they can't diverge; the panel derives its inputs (authoritative `/api/slots` when present, else the dashboard's SW-cached `offlineCapacity`). **Plus a reconnect backstop:** the fetch effect is keyed on `manualEvent.id`/date/times, so a panel left **OPEN across a reconnect never refetched** — an **online-transition effect now forces it**. **Offline lights are ADVISORY** (the device can't see customer or other-device orders); the reconnect safety net stays `CapacityBreachBanner` + never-reject replay, unchanged.

- **BACKLOG — DECISION REVERSED: sound config → PER-DEVICE (retire `trucks.sound_config`).** V8.9 recorded "**per-device master × per-truck which-sounds**". **That split is wrong:** the kitchen iPad and the owner's pocket phone want **different sounds**, and a truck-wide policy forces them to agree. Both prefs belong in the **profile dropdown, per-device — one concept, one place.** Retire `trucks.sound_config` + `set_sound_config` + its `update_settings` allowlist entry + **both** Settings panels. **KEEP** the triggers, the distinct tones, and the auto-accept new-order INSERT trigger — those are right **regardless of where the pref lives**. Same reasoning that made `keep_screen_on` per-device: **these settings are PHYSICAL.** (Supersedes the V8.9 "SOUND CONFIG" model note.)

- **BACKLOG — MOBILE SOUND CONTROL MISSING.** The header **Sound** toggle is `hidden sm:flex` (desktop only) and **UserMenu has Screen but NOT Sound** — so a **phone operator cannot toggle sound at all**; it sits at its default forever. **Mirror the Screen pattern:** same per-device localStorage, `primeAudio()` on enable, in **both** dashboard and KDS UserMenus, respecting the canonical dropdown order.

- **⚠️ BACKLOG — APP-WIDE CTA CONTRAST (fails WCAG AA).** **White on orange-600 (`#EA580C`) = 3.56:1 — below the AA floor of 4.5:1 for NORMAL text.** This is the app's canonical primary button (`bg-orange-600 text-white`, ~80 uses) — Billing's Upgrade/Choose buttons, nav buttons, **every orange filled button**, not just `/landing`. **Matters more than usual: operators read this outdoors in sunlight.** On `/landing` the PRIMARY CTAs were made to pass by going **large + bold** (`1.2rem`/`700`) so WCAG "large text" (≥18.66px **AND** 700+) applies → the **3:1** threshold, which 3.56 clears; everything else (nav buttons, Billing's buttons) stays **consciously sub-AA**. **No clean fix keeps a SINGLE orange AND a compact button:** navy label = 3.72:1 (still short); orange-700 `#C2410C` = 5.18:1 but **reintroduces a second orange**. **DECISION when addressed: it's a button-TOKEN change across the app (one shared primary-button style), not a per-page tweak** — pick one of {darken the token to orange-700 and accept a redder button everywhere, or make the shared button large+bold, or move to a navy-on-orange scheme} and apply it once.

- **⚠️ DO-NOT-RELEARN — the `overflow-x:auto` STICKY TRAP.** Setting `overflow-x:auto` on an element **silently forces `overflow-y:auto` too** (CSS: if one axis is non-`visible`, the other computes to `auto`). So a `<table>` inside an `overflow-x:auto` wrapper becomes its OWN scroll container, and `position:sticky` on the `thead` pins **inside that content-height box** — it never sticks on PAGE scroll. The landing artifact's comparison table had exactly this (`.cmp-wrap{overflow-x:auto}` + a sticky `thead`), so the header silently didn't stick. **FIX: don't use a table + overflow wrapper — mirror Manage → Billing's FLEX layout:** a `sticky top:var(--nav-h)` header row with **fixed narrow columns**. The SAME `--nav-h` variable drives the nav height AND the sticky offset, so they can't desync — no magic pixel offset. And fixed narrow columns **fit mobile with NO horizontal scroll** — which removes the `overflow-x` that creates the trap in the first place. (Same family as the earlier sticky menu-category-header regressions.)

- **`/landing` — THIRD RENDERER of `lib/plan-features.ts`, not a copy.** The public pricing/comparison page renders from the SAME source as Admin and Manage → Billing: `FEATURE_SECTIONS` (+ the now-populated per-row `FeatureRow.detail` blurbs), `PLAN_PRICES`, `PLAN_ALLOWANCES`, `TRANSACTION_ROWS`, `FOOTNOTES`. **NEW this session: `PLAN_ALLOWANCES`** (£1,500 of included online orders on Pro / £2,000 on Max, then 0.99%). The standalone HTML artifact hand-writes its own table — that markup was **NOT copied**; it's a snapshot, the source is authoritative. **Brand navy = `lib/brand.ts` `HEADER_BG`** (applied to nav + footer — single source, no drift); the lifted-navy/body colour values live in `landing.css` scoped under `.hg-landing` and are **landing-local, NOT a brand decision.**

- **⚠️ "Online ordering — Pay at Hatch" is STARTER-ONLY, and that is DELIBERATE — do NOT "fix" it to ✓/✓/✓.** It reads like a bug (why would Pro/Max *lose* pay-at-hatch?) and got queried twice. Reason: **pay-at-hatch earns HatchGrab nothing**; the paid tiers exist for **online payments (the 0.99%)**, so pay-at-hatch is the Starter offer and paid tiers are sold on the payment feature, not this row. The gate (`online_ordering_pay_at_hatch` in the starter Set only) models the business correctly. Both Billing and `/landing` render it ✓ / — / — from source. **Leave it.**

- **LANDING PAGE STATE — built on branch `landing-v32`, UNPUSHED (Gusto trading).** A SEPARATE workstream from the hardware fixes above (those are on `main`; the landing + plan-features reconciliation is not). ⚠️ **NOT ready to go public — four blockers:** (a) the hero screenshot fan is **dashed PLACEHOLDER boxes** — needs three real screenshots; (b) the **Gusto testimonial is an INVENTED placeholder with NO permission** — marked `⚠️ DO-NOT-PUBLISH` in a comment; needs their actual words + consent (the logo is real; the quote + award are not); (c) **Privacy + Terms are unwritten** (a Stripe-onboarding prerequisite); (d) the **demo** ("upload a photo, see it working in 30 seconds, no signup") is the hero's whole hook and **IS NOT BUILT.** ⚠️ **Also: the page promises Messenger/Instagram auto-replies in a headline benefit while those routes are stubs** — either the classifier ships before launch, or that line says **WhatsApp only.**

- **COPY PRINCIPLE (decided ~20 rewrites — worth keeping).** **Every claim must be true for EVERY reader, or it belongs where the reader expects detail (a footnote / the pricing table), not in a headline.** Worked examples: "No payment details needed" was **cut** (false once they onboard Stripe); "(only pay standard card fees)" was **refused at the CTA** (false for the pay-at-hatch majority) and moved to the pricing footnote; "Nothing new to buy" → **"Works on any device"** because the flagship reference (Gusto) is buying a tablet. **Same disease as "will sync" and the lying wake-lock toggle: a claim that needs a footnote to be true is in the WRONG PLACE, not in need of a footnote.**
...
**2. `/api/dashboard`'s truck response is a HAND-PICKED SUBSET (`route.ts:398`), NOT `select('*')` — a `trucks.*` field the dashboard OR KDS reads must be added to that map, or it arrives `undefined` and silently falls back to its default. Nothing fails loudly.** (Contrast `/api/manage`, which returns the raw `select('*')` row — that's why the same setting behaves differently across the two surfaces.) **[SWEPT + CLOSED in V9.0: exactly two members ever — `sound_config` + `keep_screen_on`, both fixed; `/api/manage` immune. Do not re-audit. The RULE below still governs any NEW truck-level field — the map remains a hand-maintained list that fails silently. See the V9.0 changelog.]**
...
- **RULE — adding a per-truck setting means wiring THREE things or it HALF-WORKS:** (1) the **write path** — the `update_settings` allowlist and/or the dashboard/action handler; (2) the **`/api/dashboard` truck map** if the dashboard or KDS reads it; (3) the **UI**. Missing (2) looks **exactly** like a state/reseed bug and will send you diagnosing the wrong layer (it did — the config/live split was suspected first; **it cost most of an afternoon and two wrong theories**). Verify a new field with a real `GET /api/dashboard`, not just the DB.
...
- **THE FIX — ⚠️ SUPERSEDED by V9.0 (this gesture approach was itself the next bug; see the V9.0 changelog).** *Original V8.9 fix, retained for context:* acquire on gesture — `keepAwakeOnGesture()` armed on mount and took the lock on the first `pointerdown`/`keydown`/`touchend`; **focus-retry** (`visibilitychange` does NOT fire on focus/blur, so a denial stayed stuck until a full tab-away-and-back — a `window.focus` listener heals it); an honest **BINARY toggle** (green ONLY when the lock is actually held, via the live `WakeState` subscription); and `KeepAwakePrompt` ("Tap anywhere…") as a passive bar. **WHY IT WAS WRONG:** the global one-shot listener fired on **`pointerdown`**, which **WebKit does not honour as a user-activation for `wakeLock.request`** (it honours the completed `click`), so on Safari the banner path silently failed while the real-button toggle worked. **V9.0 fix:** `KeepAwakePrompt` is a **real `<button>`** whose `onClick` runs the same `keepAwake()` path as the toggle; the global gesture listener is **removed**; `keepAwake()` calls `request()` with nothing awaited before it; mount uses `prepareKeepAwake()` (native acquires, web waits for the button). The **focus-retry, binary toggle, and UI rule below remain correct.**
...
### Feature deltas (16 July) — BUILT + committed, UNDEPLOYED

- **STOCK MODEL — CONFIRMED SEMANTICS.** A **category total is a SHARED POOL** across that category's items; an **item limit is an ADDITIONAL ceiling** within it; **whichever runs out first binds** (`calcEffectiveRemaining = min`). It is **NOT an override** — an item limit does not exempt the item from the pool. `item limit > category total` is **inert, not invalid**: **never clamp it** (silently rewriting the operator's number is worse than a harmless one — `min` binds correctly, and the stored value waits until the category is raised again). Menu & Stock UI relabelled to match (category-total column header, aligned inputs, pool-model helper text).

- **CUSTOMER STOCK COUNTDOWN — was a long-standing GAP, not a regression.** The menu API computed `stock_remaining` **per-item only** (`override ?? default_stock`), so a **category-capped menu with no per-item caps reported `null`** → customers saw **no badge AND items never visually sold out** → the customer built a basket and got **BOUNCED at submit**. Fixed by folding the **category leg** via `calcEffectiveRemaining`, using the **SAME ceilings + the same deal-aware `getLiveItemCounts`** the submit gate uses, so **"hidden on the menu" ⟺ "would be blocked at submit"** (menu API emits `item_remaining`/`category_remaining` separately + a `stock_bound` flag for copy). ⚠️ **NO BACKSTOP AGAINST OVER-HIDING:** the submit gate only blocks oversell; if display wrongly HIDES an item the customer never reaches submit and nothing catches it. **Correctness of the category leg is load-bearing — reuse the gate's sources, never re-derive them.**

- **CATEGORY-SCOPED GATE — fixed a MONTHS-OLD operator bug.** `effectiveRem` was **category-bound** while `totalInBasket` was **per-item-name**, so **4-of-each-item passed a 4-per-CATEGORY cap on BOTH surfaces** (customer + operator; operator's was pre-existing via `calcEffectiveRemaining`). Now ONE shared helper — **`calcAddableRemaining({itemRem, catRem, itemBasketQty, catBasketQty}) → {addable, bound}`** (`lib/stock-utils.ts`) — drives the badge, the `+`-disable, and **matches `checkCeilingShortfall` exactly** (proved both directions: client never allows what the server rejects, never blocks what it accepts). Badge shows **what you can still ADD** (`remaining − your basket`), so **"0 left" ⟺ `+` disabled ⟺ submit would reject**. Deal constituents already folded via the existing `basketByCat` memos on both surfaces (customer `order/page.tsx`, operator `AddOrderPanel`).

- **CATEGORY ENABLE/DISABLE — GATE, not bulk-write.** `event_category_stock.available` (migration `20260716`). Turning a category off does **NOT** write `available=false` to every item (**lossy** — reopening would resurrect items the operator had individually sold out). **One flag; readers treat its items as unavailable; item state untouched → reopening restores exactly what was there.** (The §64 order-ready master bulk-writes *because there's no per-item state to preserve*; here there IS, so the precedent argues AGAINST it.) Honoured in **FOUR read paths**: **menu API** (OMITS the items entirely, so the customer's tab vanishes via `groupByCategory` — no client change), the **SUBMIT GATE** (**mandatory** — menu-hide alone is a bypass; a **dedicated `categoryClosed` 409**, not a faked `remaining=0`), **operator Add Order** (tiles stay tappable; override via the 409 → confirm → resubmit, because staff may close ONLINE orders while still serving the hatch), and **`enforceStockLimits`** (skips disabled, else it strands items `false` on reopen). **DISABLED ≠ EXHAUSTED** — disabled hides the tab; exhausted keeps it with crossed-out items. Don't collapse them.
...
Delta over V8.7. Gusto's first live event (Ashdon Village Hall): operator-add on night 1 (14 Jul), then **online customer orders from 15 July — the first real-world exercise of the customer order path.** Two production-hidden bugs surfaced and were fixed; a batch of order-entry UX; and **two manual sections corrected IN PLACE (§61, §65)** because they confidently described realities that no longer existed and actively misled diagnosis this session — a reader must find the truth in the section, not a changelog entry.

### ⚠️ DO-NOT-RELEARN cluster

**1. "RECORDED AS APPLIED ≠ APPLIED" (worse than "written ≠ run" and "in repo ≠ deployed").** §65 recorded the `place_order_atomic` SQL rewrite as "APPLIED/live"; it was **never committed to `supabase/migrations/` and never applied to prod.** Deployed TS called the 7-param signature (no `p_draw_list`); prod had only the OLD 8-param function → **every customer `/api/orders/submit` failed with PGRST202** since the V8.0 deploy — masked because **operator orders use a DIRECT `orders` insert, never the RPC**, so operator-only usage hid a fully-broken customer path for weeks (found only when a customer test order was placed). Fixed + committed `20260715` (see §65 correction). **RULES:** a hand-run migration is "applied" ONLY when the DB says so — verify via `pg_proc`/`information_schema`/`pg_get_functiondef` and record from VERIFICATION, never intent (a "success" flag proves the statement ran, not what it did; `IF NOT EXISTS` succeeds either way); **commit every hand-run migration** (pasted-only SQL is lost — that's how this vanished); before replacing a function, **dump the LIVE definition** and base the change on that (prod may have drifted); **deploy-coupling applies to function SIGNATURES too**, not just columns; **a customer-only path can stay broken indefinitely under operator-only usage — test it explicitly.**

**2. `crypto.randomUUID()` is SECURE-CONTEXT-ONLY.** Symptom "works on desktop, fails on phone" looks like a mobile/responsive bug — it isn't. `http://localhost:3000` IS a secure context; `http://192.168.x.x:3000` (LAN-IP over plain HTTP, phone testing) is NOT → `crypto.randomUUID` is `undefined` → the fallback fires. `newUuid()` (`lib/native/outbox.ts`) feeds BOTH the outbox `op_id` AND the client-minted `order_key` (a uuid PK); its old fallback returned `op_${Date.now()}_${rand}` → Postgres `invalid input syntax for type uuid` → mobile-only 500. FIX: always return a valid v4 (`crypto.randomUUID()` → `crypto.getRandomValues()` which works in insecure contexts → `Math.random()` last-ditch). **RULES:** never fall back to a non-uuid string where a uuid column consumes it; use `getRandomValues()` for uuid fallbacks; **"works on desktop, not the phone" → suspect SECURE CONTEXT before device/responsive code** — the difference is how you reached the server, not the device. (The two-id architecture — `order_key` being a uuid column — made Postgres reject it loudly rather than store garbage.)

**3. ESTABLISH WHICH SURFACE before fixing a display bug.** The "40 left on every item" report was **Menu & Stock**; the fix was applied to **AddOrderPanel** — where the `≤10` threshold meant 40 could never show. Net: didn't fix the complaint, **killed a legitimate low-stock warning**, and desynced badge-from-gate in the mixed case (reverted, byte-verified vs HEAD). The stock badge deliberately differs by surface — **AddOrderPanel** renders `effectiveRem` (binding min, `≤10` low-stock warning, gate-aligned); **Menu & Stock** renders `itemRem` only (real per-item caps, no threshold, never echoes `catRem`). `effectiveRem` is for GATING; the display choice depends on the surface's job. **Do NOT unify; two surfaces, two rules.**

**4. "Native code is inert on web" is only HALF TRUE (branch-model post-mortem).** `ipad-native-app` became a catch-all: **58 commits / 5,560 lines of WEB work trapped** on a native-named branch with **production served FROM the branch**. Grepping imports proved almost everything is WEB-BUNDLE work → belongs on **main**, **incl. `lib/native/*` + `components/native/*`** (they compile into the browser bundle, gated no-op on web; the Capacitor app loads that bundle remotely — so "iPad work" is really web work). Genuinely native-only = **`capacitor.config.ts` + `ios/**`** (~20 files, inert on main). Resolved lossless (main was a strict ancestor → clean fast-forward `git push origin ipad-native-app:main`); **Vercel Production Branch was already `main`** (Settings → **Environments → Production**, NOT Settings → Git). **RULE: web change → main** (feature branches → main; native-only can live on main too). **DEPLOY-RISK CORRECTION:** the genuinely native bits are inert on web, but the **offline-aware refactor of the shared order-lifecycle + slots-capacity engine RUNS ON WEB** (`displaySlots` fail-safe rewrite, optimistic-capacity folding, `mergeOrders` `updated_at` guard, status overlay) — that refactor, not native-inertness, is the real deploy risk, and it's verified by code-trace not live use.

### Order notes — the asymmetric model + the safety hold

**ASYMMETRIC MODEL:** the truck can **ALWAYS** note any item (no config); `allow_notes` is now **CUSTOMER-only** (re-scoped meaning, **no schema change**). Matches POS norm (Square/Toast/Lightspeed: staff free-text any line always; customer-facing config'd). Operator modal Note un-gated; "✏️" on every added line; notes-only modal verified clean. Manage copy → "Allow customer notes" / "Truck can always note; this is for customers." BACKLOG: gating is per-CATEGORY; true per-item is a bigger data change.

**NOTES FORCE MANUAL REVIEW (SAFETY, default ON):** the auto-accept rollup (`orders/submit:799`) never consulted notes → a customer "SEVERE NUT ALLERGY" **auto-confirmed unread** (note first surfaced in the email, *after* acceptance) — against the governing allergen principle. Added `trucks.notes_require_review boolean NOT NULL DEFAULT true` (applied + verified on `ffphgwonshgxamtvefcv`, all 3 trucks true). One AND-chain term: `&& !(truck.notes_require_review !== false && orderHasNotes)` (`!== false` so pre-migration/undefined still reviews — matches the `preorders_enabled !== false` convention). **`orderHasNotes` covers ALL THREE free-text paths: order-level `notes` ‖ `items[].specialInstructions` ‖ `deals[].slotNotes`** (the last was a real hole — a customer ordering *only* a deal with an allergy note would have auto-accepted unread); reads the RAW request body (not transformed shapes) so a downstream transform can't drop the field; throw-safe on any input. **Customisations correctly do NOT fire** (`modifiers`/`slotModifiers` are separate arrays, never read — choices don't need human eyes, requests do). Reuses the existing `pending` state (no new status); **alerting inherited free** — the feature produces `!autoAccepted`, exactly what the 880Hz ding / pulsing badge / "New — action needed" section / the APNs push (`if (!autoAccepted)`, `submit:987`) key off (caveat: off-screen push needs APNs — confirm `order_pending` at native rollout). UI on BOTH dashboard + Manage, **nested under Auto-accept, shown only when auto-accept is ON**; Manage save added `'notes_require_review'` to the `update_settings` **ALLOWLIST** (saveFormField spreads the whole form; the allowlist silently drops unlisted fields). **DIRECT polarity, no inversion** (checked = hold) — an inverted label on a safety toggle risks silently disabling the hold.

### Per-line mobile order rows (both surfaces) + the customer modal EDIT port

**Operator mobile bug FIXED:** the tile displayed `totalInBasket` (SUM across cartKeys by name) while `+`/`−` touched only the base line → a customised line could become **unremovable from the menu** (displayed ≠ controlled). The `md:hidden` `menuList` now renders **one row per cart line**, stepper bound to THAT line's cartKey (displayed == controlled). **LINE IDENTITY = `makeCartKey(name, sorted-mods, note:<text>)`** (`AddOrderPanel:47`; identical copy `order/page.tsx:122`): same item + different customisation → separate line, for free; **edit is BLOCK-level (no split primitive)** across desktop/Review/customer.

**CUSTOMER gap closed + modal EDIT mode:** `makeCartKey` already encoded notes, but the flow only exposed separate lines for MODIFIER items (a plain notes item's stepper `+` inherited the note). Gates → `(hasModifiers || catAllowNotes)`. The pre-ship `commitNote` trap (it targeted ALL zero-modifier lines → would collapse multiple noted lines into one) was replaced by a per-cartKey merge-aware path, then **superseded entirely** by porting the operator's remove-and-re-add EDIT mode to the customer modal (two competing edit paths is the double-render class that produced the orphan note row — consolidate to ONE). The customer modal was ADD-ONLY → built edit mode: **`editCartKey` on `itemModal`, ALWAYS written by `openItemModal` (value or undefined) so an add-open can never inherit a stale key** (a structural guarantee — the highest-severity failure mode); `confirmAddFromModal` = clean `if (editCartKey) {edit} else {add}` (add branch byte-identical); **upsells skipped + "Goes well with" hidden in edit**; **ONE atomic `setBasket`** (no-op / merge-sum-qty-on-collision / replace-preserving-old-qty — quantity carried invisibly, block edit); **net-new-option pool guard only** (removed old line frees its own; server backstops). Pencil ⟺ `catAllowNotes || hasModifiers` (every variant row).

**MOBILE ROW LAYOUT (both surfaces):** extras/notes were TRUNCATED (second extra invisible) — **never truncate what someone chose.** Now **stacked one per line**, note beneath (📝, muted italic, `text-sm` rem), `break-words`, nothing hidden (row grows taller — honest). **Pencil-only on both** with a ~44px padded tap target (a text "Edit" button competes for the width the extras need). Desktop/iPad keeps "✏ Edit".

### WEB has NO offline order protection (deliberate) + the honest-failure fix

On **web**, offline place/confirm attempts are **LOST** — not queued anywhere. `gatedAction` (`lib/native/orderGate.ts:141,148-152`) is `isNativeApp()`-gated on both paths; `public/sw.js` is READ-CACHE ONLY (the old IndexedDB mutation queue is dead code); the offline-indicator source was native-gated (`dashboard/[token]/page.tsx:491`) → web showed no chip and a bare "Failed" toast. **DELIBERATE** — web has no durable store, so "saved, will sync" would be a FALSE PROMISE. BUILT (honest minimum, web-only): `components/WebOfflineBanner.tsx` (null on native; self-contained `navigator.onLine` + debounced `HEAD /api/ping` poll; does NOT touch the shared reachability module) + an honest submit-failure toast gated on `!isNativeApp() && !result.queued && result.status == null`. **⚠️ NEVER tell a WEB operator their offline orders "will sync" — true ONLY on the native iPad app.** BACKLOG (likely SKIP): an IndexedDB durable web queue — the iPad app IS the offline answer.

### §31 — fill distribution VERIFIED (pointer only; do not duplicate §31)

Verified against live Gusto data (3 pizzas @17:20, batch 2 → 17:15 RED 2 + 17:20 AMBER 1) = §31:3919-3937 verbatim: **intended, NOT front-loading, no engine change** (the engine is a capacity PROJECTION, not a cook schedule — §31:3923). DISPLAY confusion is real but UX-only (deferred mid-trial): dots are labelled by collection time but show the cooking window ENDING at that label → one 17:20 order lights two rows; fix direction = reframe to "headroom per collection time."

### Trial log — Gusto (Ashdon Village Hall)

**Night 1 (14 Jul, operator-add only): core held, nothing broke.** Only real issue = extras added but not attached to each dish (config friction → drove the ExtrasEditor batch). Mid-service they hit a **mobile-coverage blackspot on the WEB portal and stopped taking orders** — pre-orders placed earlier (in signal) were safe on the server; they fell back rather than order into the void (likely nothing lost). **The web portal fundamentally cannot operate in low/no coverage — this is exactly why the iPad app exists, now validated at a real venue. Rural/dead-zone coverage is the CORE OPERATING CONDITION for this market, not an edge case.** **15 Jul: taking ONLINE customer orders** — working only because the `place_order_atomic` mismatch (§65) was fixed that morning.

### ExtrasEditor batch

Per-dish grid FIXED (first colgroup `<col>` had no width under `table-layout:fixed` → DISH column collapsed → "DISH"/"OFFER" overlapped as "DOSFHFER" + dish names clipped; fix = explicit `width:200` + table `min-width = 200+56+optionCount*76` scrolls in the existing `overflow-x-auto` + `title` attr). "Offer" → "Include". Select-all per category (tri-state ✓/—/empty, `stopPropagation`, capability-gated; per-COLUMN skipped — columns are options, meaningful only once a dish is offered the group). "Offered on" collapses by category (`n===m` → "Pizza"; `n<m` → "Category (n of m)" when `m>4` else dish names; `*` if any dish hides options).

### Infra — Vercel/Brevo free tiers (NOW live-relevant)
...
**iPad OFFLINE RESILIENCE (dead-zone venues, the proven core requirement):** pre-cache event/menu/capacity/stock while still in signal; persistent "operating offline — N queued" state; manual "sync now". HONEST LIMIT: customer *online* ordering cannot work in a true dead zone → dead-zone venues are operator-add-only.
**Small cleanups:** remove the orphaned `hideName` prop on `OrderLineItem` (the mobile row no longer uses that component); remove the now-unused `subLabel`/`modLabel` locals on both mobile surfaces.

## V8.7 — July 2026

Delta over V8.6. One arc: getting the iPad app to actually work end-to-end (a long login-loop debug), then making the WHOLE operator+admin product self-contained in the app — dashboard + KDS + manage + admin + sign-out, no browser bounces. Every fix is **ADDITIVE + native-gated**; every shared server route was **curl-proven web-safe** (no-header/web response byte-for-byte identical to before; invalid Bearer still denied; valid native Bearer authorizes). **Status: BUILT, tsc-clean, UNDEPLOYED.** Simulator-testable now (login → dashboard → KDS in-app → truck switch → manage → admin console → sign-out, all self-contained); still hardware-gated for APNs, real Face ID, cold-launch, real network, and WKWebView-session-survives-hard-KILL.

### The login loop — root cause (the headline lesson)

Symptom: in the native app EVERY account (admin + operator) looped — login → `/app` landing SUCCEEDS (session valid, my-trucks 200) → routes to `/dashboard/[token]` → bounces back to the email/password login → repeat, unbreakable. Web login fine.

ROOT CAUSE: **in Next.js 16, `middleware.ts` is renamed `proxy.ts`.** The auth guard lived in `proxy.ts` — a cookie-based `supabase.auth.getUser()` that 307-redirected `/dashboard` + `/manage` to `/login?next=<path>`. It was invisible because (a) grep for "middleware" missed the renamed file, and (b) the `?next=` was built via `searchParams.set('next', pathname)`, so `/login?next=` never appears as a string literal in source or the compiled bundle. The native app has NO cookie → `user` null → 307 → loop; logging in creates a NATIVE (Preferences/Bearer) session, never a cookie, so the guard fails again on return = unbreakable loop.

**LESSON (do-not-relearn):** on any "is there a middleware/guard doing X?" question, check `proxy.ts` (Next 16), not `middleware.ts`; middleware manifests in `.next` can be stale — trust `curl -i` over the manifest. A server redirect built via `searchParams.set(...)` is not grep-able as a literal — trap it at runtime (a `history.pushState`/`replaceState` patch capturing the stack trace is what finally caught it; persist to @capacitor/preferences to survive a hard reload).

FIX: `capacitor.config.ts` → `ios.appendUserAgent: 'HatchGrabNativeApp'`; `proxy.ts` guard now `if (isProtected && !user && !isNativeApp)` where `isNativeApp` = UA includes the marker. Native navigations are deferred (middleware stops hard-redirecting) → auth enforced downstream (the client page checks the native session + sends the Bearer to `/api/dashboard`). Web (no marker) → 307 exactly as before (curl-proven identical). **The UA marker is read at WKWebView init from the baked config → needs a fresh binary (Xcode rebuild), not just a dev-server reload.**

### Native-session fixes found en route (ruled-out loop theories that were real bugs anyway)

- **CapacitorHttp DISABLED** (`capacitor.config`): it patched fetch and broke RSC-payload navigations ("Failed to fetch RSC payload / CapacitorUrlRequestError 0"). A remote-URL shell wants the WebView's own networking; no code depended on it.
- **Session persistence → @capacitor/preferences.** `getNativeSupabase()` used localStorage, which didn't reliably survive WKWebView navigations → session lost after login (looked like session loss). Swapped storage to a @capacitor/preferences adapter (`lib/native/preferencesStorage.ts` → iOS UserDefaults); one line in `getNativeSupabase` (`storage: preferencesAuthStorage`); `persistSession`/`autoRefreshToken`/`storageKey` unchanged. **Preferences is a NATIVE plugin → adding it (any native plugin) needs `npx cap sync ios` + an Xcode rebuild, not just a dev-server reload.** (NOT the loop cause — proxy.ts was — but the correct, more durable storage, kept.)
- **KeepAwake "then() not implemented on ios":** `getPlugin()` returned the raw Capacitor plugin proxy, which looks like a thenable → `await` assimilated it → native invoke of a non-existent `then`. Fixed: return a plain `{ KeepAwake }` object (`lib/native/keepAwake.ts`). Was a fire-and-forget non-awaited call → cosmetic, never blocked the flow.
- **Bundle ID reconciled to `com.hatchgrab.app`** (was `co.villagefoodie.app` in the Xcode project vs `com.hatchgrab.app` in capacitor.config — Xcode wins). Changed `PRODUCT_BUNDLE_IDENTIFIER` (Debug + Release). CANONICAL ID for APNs / App Store = **com.hatchgrab.app** (`APNS_BUNDLE_ID` + the Apple App ID must use it).

### Full native operator+admin experience ("no leaving the app")

The shared pattern, applied consistently: **(A)** cookie-blind API → additive Bearer branch (cookie `getUser` runs first + resolves for web; the Bearer branch is `!user`-AND-header-gated, so browsers never enter it); **(B)** client fetch → add `nativeAuthHeader()` (returns `{}` on web, so no header added); **(C)** hard `window.location`/`_blank` nav → make native-aware. Every server change curl-proven web byte-for-byte unchanged.

- **/api/dashboard native Bearer acceptance:** no cookie user + valid native Bearer → resolve operator + `is_admin` → app-admin gets `userRole:'owner'` (owner-level UI, all-truck access). Cookie path unchanged; dashboard + KDS clients send the Bearer.
  - *(**⚠️ CORRECTION — V9.2:** this and the V8.6 architecture note ("role defaults to owner without the cookie") have been read as a GENERAL property of the native shell — *any* token-only native session resolving to `'owner'`. **It is not.** `/api/dashboard` runs **ONE role block for both the cookie and the Bearer path** (`app/api/dashboard/route.ts`), so a staff member on the native app correctly resolves to `'staff'` — the app-admin branch above is an ADMIN bypass, not a token-only default. The `|| 'owner'` fallback is **the MANAGE page's, client-side** — `app/manage/[token]/page.tsx`, where a token-only session with no resolved role falls back to owner-level UI. **Cite the manage page for that behaviour, never `/api/dashboard`.**)*
- **/dashboard token-less INDEX guard** (`app/dashboard/page.tsx`): its cookie `if(!user) redirect('/login')` is native-blind → `DashboardIndexNativeFallback` (client): native + `hasNativeSession` → `/app`, else `/login`. Web unchanged.
- **my-trucks admin bypass:** `permittedTruckIds` returns ALL active trucks for `operators.is_admin` (an admin who owns no truck still gets trucks; previously `[]` → landing bounce).
- **/admin console:** `verifyAdmin(req?)` + `/api/admin/create-operator` + `/api/auth/me` got the additive Bearer branch; the dashboard's `/api/auth/me` fetch + all 9 admin-page fetches send `nativeAuthHeader()`. `is_admin` now flows to the app → Admin link shows in UserMenu → console loads (not "Access denied"). `verifyAdmin` is shared by every `/api/admin` handler AND the web console — the additive branch was curl-proven to leave the web admin path byte-for-byte unchanged. (Truck-switching already sufficed to reproduce operator issues via the native switcher + owner bypass; `/admin` is only for platform-admin: discovery config, create-operator.)
- **/manage:** core (menu/schedule/settings/deals) already worked in-app via the token (`/api/manage` is token-based → owner). TeamTab profile/email needed the 4 `/api/auth/*` mutations (`update-profile`, `change-email`, `cancel-email-change`, `resend-verification`) + `/api/auth/me` Bearer-enabled + `nativeAuthHeader()` on the TeamTab fetches. Curl-proven web-safe.
- **Native sign-out:** NEW `lib/native/signOut.ts` `operatorSignOut(router)`. Native → `nativeSignOut()` (clears the Preferences session — the REAL bug: manage/admin previously called cookie `signOut()` which is a no-op in the app, so the user was NOT actually signed out) → soft `router.replace('/login')` (stays in the webview). Web → unchanged (cookie signOut + `window.location`). Wired at all live sites (UserMenu, manage, admin's Access-denied screen); dead `dashboard/[token]` handler deleted.
- **KDS "Kitchen screen":** was `window.open(...,'_blank')` → in WKWebView `_blank` escapes to Safari / no-ops (button broken in-app). Native → `router.push` the in-app `/dashboard/[token]/kds` (dashboard_token-based, authenticates natively; van via `?van_id=&van_name=`). Web keeps `_blank`. All 4 KDS `_blank` sites converted. Genuinely-external `_blank`s (schedule_url, "View original card") left as-is.

### DEPLOY RISK — read before deploying

This batch added additive Bearer branches to **`proxy.ts`, `/api/dashboard`, `verifyAdmin` + all `/api/admin*`, `/api/admin/create-operator`, `/api/auth/me`, and 4 `/api/auth/*` mutations** — plus the `/dashboard` index fallback. These are the routes EVERY dashboard/admin/manage load hits. Each is individually curl-proven web-safe in dev, but "additive + tsc-clean ≠ correct-when-rendered on the deployed build." On deploy, run a **both-paths smoke test on the DEPLOYED build:** (1) web cookie path still guards/resolves for a real web operator AND the web admin console; (2) native Bearer path works. `proxy.ts` especially — it runs on every `/dashboard` + `/manage` request.

### App state / testing

Diagnostics all removed — the real `/app` landing auto-routes (session → my-trucks → `router.replace` to the truck, brief "Loading…", no Continue button). Localhost testing config: `CAP_SERVER_URL=http://localhost:3000 npx cap sync ios` (revert = plain `npx cap sync ios` → production).

### Addendum — scroll / safe-area (WKWebView)

- **Page scrolling.** `capacitor.config` `ios.scrollEnabled` was `false` — traced to INITIAL-SCAFFOLD BOILERPLATE (first commit `03d788f`, when the app was `co.villagefoodie.app`), NOT a deliberate bounce fix. Flipped to `true` → the three `min-h-screen` natural-flow pages (Dashboard, Manage, Admin) now scroll like web. KDS was always fine (`h-full flex-col overflow-hidden` + inner `flex-1 min-h-0 overflow-y-auto` — the correct webview pattern, the footer-drop lesson done right at page level). Fallback if it ever bounces: cap the three pages to `h-dvh flex-col` + `flex-1 min-h-0 overflow-y-auto` mirroring KDS, native-gated. Prefer `h-dvh` over `h-screen`/`100vh` in the webview.
- **Top safe-area bleed.** Enabling scroll exposed light page content bleeding into the status-bar strip above the dark sticky `AppHeader` (`contentInset:'always'` + `scrollEnabled` double-insetted). Fix = the immersive-header pattern, CSS owns the inset: `AppHeader` gets `paddingTop: env(safe-area-inset-top)` (dark bg fills the strip), `app/layout` viewport `viewportFit:'cover'` (populates `env()`), `capacitor.config` `contentInset` `'always'`→`'never'` (OS stops auto-insetting; CSS `env()` solely owns it). All inert on web (`env()`=0, viewport-fit no-op, contentInset iOS-only).
- **Bottom-inset caveat.** `contentInset:'never'` also drops the OS bottom inset → content can run under the home indicator on long lists. If it bites, add `padding-bottom: env(safe-area-inset-bottom)` on the scroll containers (pattern already at `order/page.tsx:2164`). Check on device.

### Addendum — navigation sweep (everything stays in-app)

Hit the "escapes to Safari" bug three times (KDS `_blank`, sign-out `window.location`, admin→dashboard link) → did a systematic sweep. NEW `components/native/AppLink.tsx`: a drop-in `<a>` replacement — native intercepts the click → `router.push` (in-app); web renders a byte-identical plain `<a>` (same href/target/rel). Any future internal link uses `<AppLink>` and this class can't recur.
- **INTERNAL routes converted:** admin (`/dashboard/[dashboard_token]` — the reported bug, was `<a target="_blank">`), KDS "← Dashboard", KDS "Open cook screen" (was `_blank`), dashboard "Edit categories" (→ `/manage`), manage "← Orders dashboard". Already correct (next/link or prior `openKDS`/`operatorSignOut`): UserMenu Manage/Admin, KDS button, sign-out.
- **EXTERNAL left as plain `<a target="_blank">` → Safari (correct, off-platform):** truck `schedule_url`, manage "View original card" (external allergen-card image). BACKLOG: Dominic unsure what `schedule_url` / the allergen card are — revisit; optionally route these through an in-app browser (Capacitor Browser plugin) for a fully-sealed experience instead of Safari.

### Addendum — call-site completeness (Admin link)

`is_admin` reaches the app UI only via `/api/auth/me` (NOT `/api/dashboard`). Both call sites now send `nativeAuthHeader()`: dashboard (`:457`) AND manage (`:293`) — manage was the one missed, so the Admin link was hidden on the manage page while showing on the dashboard, now fixed. **Lesson:** when a flag rides one endpoint fetched from multiple pages, wire the Bearer at EVERY call site.

### Addendum — AGREED offline order-handling design (NOT built; no code, no migration)

Dominic prioritised this for retention/parity (accepts competitor capability is ASSUMED not verified; offline stock can't be 100% — "rare, so leeway"). **The safety case is ALREADY covered server-side by heartbeat auto-pause** (blocks new customer orders when a truck goes offline); this build is parity/UX, NOT a safety requirement. Full lifecycle audit done.

KEY SCHEMA FACTS: order has `order_key` (uuid, true identity, collision-safe, client-can-generate) + `id` (text, server-assigned sequential-per-event DISPLAY number via `incr_event_order_counter` — a client-generated display id WOULD collide); `place_order_atomic` is one transaction; auto-accept server-authoritative (`status='pending'` = needs confirm); auto-pause blocks new customer orders but does NOT freeze customer CANCELS (so operator-offline-status-change vs customer-online-cancel is the real conflict); NO change-log / reliable `updated_at` (last-write-wins NOT safely implementable as-is); capacity/stock is a derived-rebuild aggregate (self-heals but over-acceptance already happened); existing `public/sw.js` (KDS-only) already does offline read-cache + a CONFLICT-BLIND mutation-replay queue (latent-unsafe in live code — backlog).

AGREED DESIGN (not built): (1) offline STATUS CHANGES queue, page behaves identically + offline banner, sync on reconnect. (2) offline ORDER CREATION = operator walk-up orders on the iPad only (NOT customer online ordering, which can't reach an offline truck): device-PREFIXED provisional numbers (per-device letter from `device_id`, e.g. A13/B13, no collision) + local capacity/stock countdown from last-known snapshot + "offline — stock approximate" warning (rare over-sell flagged not prevented). (3) RECONNECT: apply safe-default automatically (timestamp rule where applicable; **customer-cancel-wins** for the conflict case) + mark genuine conflicts with "!" for review-at-leisure — NO forced mid-service adjudication (non-blocking "back online" notice). (4) PREREQUISITE: a change-log table (reliable status-change history) — schema lacks it. (5) fix the existing conflict-blind SW replay to be conflict-aware. NOT day-1 (auto-pause covers the new-order safety case; outages rare). **This is the biggest pending native build.**

### Addendum (2) — responsive layout is now WIDTH-KEYED (tablet-first)

Operators use TABLETS, not desktops, so the tablet layout must be consistent whether in the app OR Safari-on-iPad — the trigger is **VIEWPORT WIDTH, not `isNativeApp()`**. (LAYOUT keys off width; BEHAVIOUR — in-app nav, native sign-out, Bearer — still keys off `isNativeApp()`.) Boundary = **1400px** via a custom Tailwind `min-[1400px]:` arbitrary variant — no stock breakpoint fits (xl=1280 would wrongly center the 13" iPad at 1366; 2xl=1536 would wrongly full-bleed a 1440 desktop). Bands: **MOBILE <768** (existing mobile layouts, unchanged) · **TABLET 768–1399** (full-bleed `w-full` + side-by-side controls; app AND web) · **DESKTOP ≥1400** (centered `max-w-5xl/6xl mx-auto`, byte-for-byte as before). Width caps converted: dashboard ×3, AppHeader, manage ×2, admin ×2 → `w-full min-[1400px]:max-w-5xl/6xl min-[1400px]:mx-auto`; control row `lg:`→`md:` (side-by-side from 768). Accepted tradeoffs: a 1366px Windows laptop reads as tablet (same width as a 13" iPad — unavoidable); 1025–1399 desktop windows go full-bleed. **This changes web at tablet/narrow-desktop widths → on deploy, eyeball web at those widths (desktop ≥1400 is unchanged).** (Superseded the earlier `isNativeApp()`-gated attempt.)

### Addendum (2) — Add-order is a fixed-viewport POS screen (both orientations)

Add-order required scrolling to reach Confirm. **ROOT CAUSE: a REGRESSION from our own safe-area fix** — a brittle `style height: calc(100dvh - 130px)` magic offset; the 130px desynced when the immersive-header work (`env(safe-area-inset-top)` + `contentInset:'never'`) made the header taller → the panel overflowed the now-scrollable page → Confirm below the fold. FIX: **removed the magic number entirely**; converted the Add tab to a true fixed-viewport **flex fit** (KDS pattern) that self-sizes to any header/inset height. **Scoped to the ADD TAB ONLY** — root `h-dvh flex-col overflow-hidden` when `activeTab==='add'`, else `min-h-screen`; Orders + other tabs untouched and still scrollable. **Landscape (≥1024/lg):** two-column — left menu `flex-1 min-h-0 overflow-y-auto`, right cart `flex-1 min-h-0 overflow-y-auto` + `submitPanel` `shrink-0` (Confirm pinned). **Portrait (<1024, incl. 11" iPad 810/834):** single-column menu fills the viewport + always-visible "Review order →" bar → Confirm bottom-sheet (reuses the phone flow; also fixes the cramped 810px two-column). Edge: 13" iPad portrait = exactly 1024 → lands in the two-column band (eyeball for cramping).

### Addendum (2) — LESSON: prefer flex-fits over hardcoded pixel offsets

Hardcoded pixel offsets (`calc(100dvh - Npx)`, fixed heights) are FRAGILE — they desync when related layout/chrome changes. This arc had TWO such downstream regressions (scroll-enable → top-inset bleed; inset change → Add-order overflow). Durable approach: **flex-based fits** — `h-dvh flex flex-col overflow-hidden` shell → `flex-1 min-h-0 overflow-y-auto` scroll region(s) → `shrink-0` pinned footer — which self-adjust to any chrome/inset height. Prefer `h-dvh` over `h-screen`/`100vh`. **Do NOT reintroduce magic-number offsets.**

## V8.6 — July 2026

Delta over V8.5. Two arcs this session: (A) the tail of the allergen/wizard batch — the "Save & add to menu" hang, five allergen/import correctness fixes, plus a card-mode warning fix and a schedule-import geocode-timeout fix surfaced by DEPLOYED-BUILD testing; (B) a full iPad operator-app Phase 1 build (mostly built onto an already-committed Capacitor app). **Status: the allergen/wizard/warning/geocode work is validated on the working build and (partially) on the deployed build; the iPad app's native parts are BUILT-BUT-HARDWARE-GATED — unvalidatable until a physical iPad + paid Apple Developer account exist. Two new migrations pending. Two new backlog items (one security).**

### The hang + the STILL-OPEN latent handler bug (carried from V8.5, re-confirmed)

"Save & add to menu" hung forever. ROOT: commit-menu `INSERT`s `modifier_groups.hide_name`; the column didn't exist yet → every custom-extra group insert failed → commit-menu returned `{ok:false}` at HTTP 200 (not a throw). `handleCommitMenu` has NO `else` on `if(data.ok)` and NO `finally` → `importStep` stuck `'saving'` → silent infinite spinner. Unblocked by running the `hide_name` migration (done by hand, deploy-coupled, first step of the deploy order). **LATENT HANDLER BUG STILL NOT FIXED (HIGH backlog):** the missing else/finally + swallowed `{ok:false}` turns ANY future commit failure into the same invisible hang. Durable fix (`finally` clears spinner + toast on non-ok) still queued. NOTE: commit-menu is best-effort/non-transactional → partial writes on failure; a naive retry double-inserts. Clear partial rows before retrying.

### Five allergen/import fixes (V8.5 arc — all built, tsc-clean, LIVE-validated on test-truck via clean re-import)

1. **Card-only mode = verbatim, no AI extraction (Option A).** Free-form allergen card is the operator's VERBATIM text shown to customers — NO vocab extraction, NO chips, NO rewrite (typed "lactose" stays "lactose", not "Dairy"). New `CardOnlyEditor` (separate from `CardUploadPage` so card-mode can't leak into per-dish): paste→verbatim to `trucks.allergen_info_text`; image→AI TRANSCRIBES allergen/dietary text faithfully (prose, preserve wording, NO vocab-map) into the SAME editable box (operator reviews/edits, never auto-saved; image consumed for transcription only, never stored, `allergen_info_url` not written in card-only). Mechanism: process-allergens gained opt-in `mode=transcribe` (text/plain, `{ok,text}`); default JSON/vocab path byte-for-byte unchanged → per-dish unaffected. Old chip flow (`cardPanel`, `showAllergenModal`, `handleSaveAllergens`, `handleAllergenUploadAndProcess`/`handleAllergenTextProcess`/`allergenExtracted`) now ORPHANED — left in place, flagged for dead-code sweep. Per-dish card→dish matching path UNCHANGED.
2. **AI custom-extra over-split → content-based consolidation.** process-menu emits one modifier group PER DISH (`_inferredFromVariants`); the V8.4 naming pass numbered identical groups apart, disabling commit-menu's name-based dedup → 6 near-dup "Mains - Protein 1…5" groups. FIX (client-side, in `computeGroupingRows`, before naming): consolidate by content SIGNATURE = (normalized category, sorted order-independent option-set [`normName#price#allergens#dietary`], rules `is_required`/`singleSelect`). EXACT MATCH ONLY (Option X — no superset, no per-dish exclusions, no cross-dish option mixing; price in the key so price-divergent groups stay separate — a shared group holds one price per option). Allergens IN signature (over-warn-safe). commit-menu name-dedup kept as backstop (now fed intentional content-names). `buildGroupedItems` re-runs pure `computeGroupingRows` at commit → render==commit. Operator never sees group names/structure — only the grouped/separate per-dish choice; consolidation is pure machinery.
3. **Variant option allergen BLEED killed + options BLANK + inferred groups REQUIRED.** Dish allergens/dietary bled onto every variant option (`ungroupAiVariantsForReview` unions dish∪option at :1959-1960 → `makeGroupingRow` copies onto each option at :2002) → "Chicken" wrongly tagged Molluscs/Vegetarian/Gluten. FIX: options commit `allergens:[]`/`dietary:[]` at the option-mapping source (:2002) — the :1959-1960 union deliberately NOT killed (feeds dish-level union + separate-path real dishes; killing it drops a curry's gluten = under-warn). DECISION: importer-populated option allergens are a FUTURE feature (`modifier_options` has NO verified flag), so ALL variant options blank. Inferred-variant groups default `is_required=true` + single-select (min1/max1) — protein is structurally must-choose; operator can amend. Side-effect: blank options + uniform required → previously-bled-apart identical groups now consolidate. Explicit "add extras" (`_inferredFromVariants:false`) option allergens RETAINED in DB as head-start for the future per-option review — now fully HIDDEN (see #5 email-strip; no other customer surface renders option allergens: pill removed V7.8 §31, basket removed §25).
4. **(subsumed into #3's required + consolidation).**
5. **Allergens REMOVED from ALL emails.** Option allergens (AI, ungated) were the last customer-facing surface, via confirmation/ready EMAIL ("— contains: X" `lib/email.ts:31` / "(contains: X)" :254 in `renderOrderLinesHtml`, shared by `formatConfirmationEmail` + `formatNewOrderEmail`). DECISION: allergens never appear in any email — items/options only. Stripped both render sites (HTML + text); modifiers still render name+price. Grep-confirmed allergen rendering existed ONLY at option level in emails (no dish-level; none in cancel/reject/new-order/ready). RENDER-ONLY (`EmailOrderItem` fields kept; data travels on order, never rendered). No DB/migration/Gusto touch.

### Card-mode "Allergens not set" warning — display-mode-awareness fix (FIX, deployed-build issue)

Surfaced on the DEPLOYED build: a truck in `display_mode='card'` with a saved card still showed "27 dishes need review — customers can't see allergen info." DIAGNOSED: the manage-side warnings key purely on the per-dish `allergens_verified=false` count and never check `allergen_display_mode`. The customer-side per-dish hide gate (menu route:435-438, `!== 'card'`) was ALREADY correct → NO customer bug; confined to four operator-side surfaces. FIX: gate all four on card-mode, mirroring the menu-API gate — (1) Menu-tab box (`needsReview` → card+saved-card = calm; card+no-card = "add a card" CTA, NOT "N dishes need review"); (2) cross-tab banner (`allergensUnverified`, :173); (3) "Menu (!)" badge (:390); (4) vestigial menu-API `allergensVerified` flag (:590, no live consumer, gated for consistency). Per_dish/both/null modes UNCHANGED (every expression reduces to its original form). Cross-tab banner + badge suppress in card mode regardless of whether a card is saved (the Menu box owns the "add a card" CTA). Non-dangerous (over-warns operator, never under-protects customer).

### Schedule-import geocode TIMEOUT (FIX, deployed-build issue)

Surfaced on the DEPLOYED build: "Import schedule" Save hung on "Saving…" for MINUTES. DIAGNOSED: unlike commit-menu, the three schedule-save handlers HAVE try/catch/finally — so an error can't hang them; only an unsettled await can. The geocode fetch (`app/api/manage/geocode/route.ts:25` → Gemini) had NO AbortController/timeout; a stalled Gemini call never returned → the client await hung → `finally` never ran. FIX: wrapped the Gemini fetch in a 10s AbortController; on timeout it aborts → falls to the existing catch → returns graceful `{lat:null,lng:null}` → the event saves WITHOUT coordinates (geocode is best-effort, already non-blocking). A stalled Gemini now resolves the save in ~10s, not minutes. Broader latent risk noted: other untimed external fetches would still hang — a systematic AbortController pass is a future hardening item (not done).

### iPad operator app — Phase 1 (BUILT; native parts HARDWARE-GATED)

Decision rationale (recorded): app is for competitive PARITY (competitor ships an iPad app) + perceived PROFESSIONALISM for trucks — NOT a safety requirement. KEY FINDING: the offline-protection safety property (pause customer ordering when the operator device can't receive orders) is ALREADY enforced SERVER-SIDE — the `heartbeat-monitor` edge function pauses an event when `truck_vans.last_heartbeat_at` goes stale (device→server silence, not device-side background execution). So the native app is NOT required for the safety guarantee; its value is reducing false pauses + order notifications. iOS reality (researched July 2026): reliable backgrounded connectivity detection is NOT fully achievable on iOS by any app — Phase 2 (deferred) is "reduce false pauses / improve awareness," never a guarantee.

**Architecture:** SINGLE app `com.hatchgrab.app` (truck-only; customers use the website — no customer app). Remote-URL Capacitor shell: the native webview loads the LIVE site (`server.url` → `/dashboard`), Vercel stays the backend. This sidesteps the wrap problems (36 API routes, relative fetches, cookie login, server components) because the origin is the real server. Token-in-URL is the credential; role defaults to owner without the cookie → cookie/session persistence is NOT load-bearing (the flagged "fiddliest risk" is a non-issue by architecture). Web iteration loop unchanged (UI deploys via Vercel); only native-shell changes need the Xcode/TestFlight/review loop.

**Built (packages):**
- Shell: entry → `/dashboard`. Deps added: `@capacitor/app`, `@capacitor/push-notifications` (^8).
- Capacitor pattern: repo already uses static `@capacitor` imports guarded by `isNativePlatform()` with web fallbacks (`lib/native/*`) — no runtime shim needed.
- **Device↔van binding (Package 3):** NEW table `van_devices` keyed on `device_id` (PER-DEVICE, not per-login — one login owns many device rows; single login + multiple devices is expected/allowed). `device_id` = durable localStorage UUID (first launch). Bind endpoint `POST /api/native/bind-device` (dashboard_token authorises; binds any device_id ONLY to a van where `van.truck_id === token's truck` — truck-ownership scoping is the security gate). Per-device config: `default_screen` (dashboard|kds), van, `notify_enabled`. First-launch `DeviceSetupGate` (role-agnostic, native-only, `isNativeApp()`+unconfigured — always asks the SCREEN question; single-van auto-assigns the van SILENTLY; multi-van requires an explicit van pick, pre-filled only for identifiable single-van staff).
- **"This device" settings home:** the `UserMenu` header dropdown (dashboard) AND a minimal header button on the KDS — both native-only, role-agnostic (before the role-gated Manage link; NOT role-gated — staff who can't reach manage>settings must configure their own device), NOT `sm:hidden` (shows on iPad at all widths). Shows default screen / van (only if >1 van) / notifications, with "these settings apply to this device only" subtext. manage>settings was REJECTED as the home (owner/manager-only).
- **Keep-awake (Package 4):** single visible "Screen on" control, dual implementation switched by runtime (invisible to user) — app → per-device localStorage pref `'hg_keepawake'` (default ON, manual off; truck setting can't override in-app); web → `keep_screen_on` as before. Only one mechanism fires in-app (the duplicate in `OperatorDeviceConfig` was removed).
- **APNs push (Package 5):** the server push is the SOLE order-notification source. Trigger: orders/submit, post-save, if `status='pending'` (needs confirming — auto-accept is server-authoritative: `truck.auto_accept`, truck-wide + per-item, materialized as status), resolve van via `order.event_id → truck_events.van_id` → van pref → `van_devices` (`notify_enabled` + `push_token`), `sendOrderPendingPush` — FIRE-AND-FORGET (same non-blocking try/catch as the confirmation email; NEVER blocks/hangs the order save). DUPE REMOVED: KDS `notifyNewOrder` local notification removed (was foreground-only + the dupe source); swapped to `playDing()` in-app sound (UI feedback, not a notification). `lib/apns.ts` = ES256 JWT + HTTP/2 sender, safe NO-OP until `APNS_*` env set. Per-van config table `van_notification_prefs` (`type='order_pending'` only for now; van-level master + per-device `notify_enabled` opt-out). Null-van / no-token → graceful no-op, order still saves.
- **Network (Package 6):** dashboard `deviceOnline` now uses the native reachability listener; foreground ping via `@capacitor/app`.

**HARDWARE-GATED (built, CANNOT validate without APNs .p8 cert + physical iPad + paid account):** APNs send end-to-end, native push registration/token delivery (simulator won't deliver a real token), tap→deep-link, and Package 1 cold-launch routing (open to the device's default screen for its bound van via persisted token — native Swift/landing work in `ios/`, not done blind). Also required on the Mac: `npx cap sync ios` + pod install (register the two new plugins), the Push Notifications capability/entitlement, and `APNS_KEY_ID`/`TEAM_ID`/`BUNDLE_ID`/`KEY`/`ENV` env. Until `APNS_*` is set, `sendOrderPendingPush` is a logged no-op — the trigger ships safe.

**Cold-launch KDS constraint (flag for Package 1 build):** the interactive KDS currently always runs on the dashboard_token (the `/kds/[kds_token]` route is a server redirect to `/dashboard/{token}/kds`). `ThisDeviceSettings`/bind-device authenticate via dashboard_token, so no mismatch today. IF cold-launch routing ever renders a KDS on a RAW kds_token without the redirect, the bind-device auth mismatch reappears — cold-launch must route through the dashboard_token.

**Testing plan:** dev iPad = 2nd-hand 10th-gen, WiFi-only (£185; A16 not worth the premium for a webview app; 10th gen supported ~through 2028-2029). WiFi-only gaps (network-transition behaviour, APNs under mobile, keep-awake battery/thermal) closable via phone-hotspot testing. Pay for the Apple Developer account at first real-device/TestFlight test (NOT rollout) — the 4.2 review outcome is the real go/no-go and can't be reached on the free account/simulator. App Store 4.2 "minimum functionality" is the top rejection risk for a remote-URL shell; mitigations (required, not optional, since store presence IS the goal): push/local notifications, native network detection, keep-awake, framed as an operator/kitchen tool.

### iPad app — persistent login + truck/van switching + biometric app-lock (built after the initial Phase 1 section; simulator-testable except where noted)

Added a real login + switchable device→truck binding + optional device lock, on top of the earlier token-in-URL wrap. ARCHITECTURE PRINCIPLE: **LOGIN** = who the person is + which trucks they may access (per-person, persistent). **DEVICE CONFIG** = what this device currently shows (truck/van/screen, per-device, remembered, switchable to any truck the logged-in user is permitted). Switching truck re-points the device; it does NOT log out. Logout clears the SESSION only, never the device config (device stays pinned; next person lands on the same config).

**Persistent login (plan a — wired):**
- Reuses existing Supabase auth (no parallel system). In-app login (`app/login/page.tsx`) uses a NEW native Supabase client (`lib/native/session.ts`, `getNativeSupabase()` singleton): `createClient` with `persistSession` + `autoRefreshToken`, localStorage-backed (`storageKey 'hg-native-auth'`). Web unchanged (cookie client).
- Log in ONCE, stays logged in across cold-launch, auto-refreshes, cleared only on explicit logout (kitchen-screen "immediate access" requirement).
- Native landing (NEW `app/app/page.tsx`; `capacitor.config` `server.url` → `/app`): no session → `/login`; session → `GET /api/native/my-trucks` → route to the device's pinned truck's `default_screen` (`/dashboard/[token]` or `…/kds`); unconfigured/non-permitted-pin → first permitted truck (DeviceSetupGate binds); no trucks → `/login`. Client-side because the server component can't see a localStorage session.
- Logout: `UserMenu` `handleSignOut` → `nativeSignOut()` (native) / cookie signOut (web) → `/login`. Does NOT touch `van_devices`.
- ⚠️ ON-DEVICE TEST REQUIRED: session persistence uses the localStorage default store. WKWebView localStorage USUALLY survives cold-launch but has hard-kill edge cases. MUST test on the physical iPad: fully quit while logged in → reopen → still logged in? If flaky, swap the client to a `@capacitor/preferences` storage adapter (the `getNativeSupabase` singleton isolates this change). A session that doesn't survive hard-kill = operators unexpectedly re-logging-in (the friction we're avoiding).

**Truck/van switching + server-side membership gate (plan b — wired):**
- `GET /api/native/my-trucks` — Bearer-authed (`supabase.auth.getUser(jwt)`, since the native session is localStorage not a cookie). `permittedTruckIds` = owner (operators→`trucks.operator_id`) ∪ member (`truck_users`), mirroring `/dashboard/page.tsx`. Returns permitted trucks (+ vans + dashboard_token) + the device's current pin (only if pinned to a permitted truck). Server enumerates membership; the client NEVER filters.
- `POST /api/native/switch-truck` — Bearer-authed; GATE: target ∈ permitted (else 403); resolves a van of the target; UPSERTs the single `device_id` row (`onConflict device_id` → in-place UPDATE of truck_id+van_id+last_seen; push_token/default_screen/notify_enabled NOT in payload → preserved). Returns the new dashboard_token.
- CROSS-TRUCK LEAK PREVENTION: one row per device_id, updated on switch, push_token carried. A device can NEVER appear under two trucks' vans → can't receive two trucks' notifications. (Verified: the switch is an UPDATE, not a second insert.)
- `ThisDeviceSettings` hierarchy: "You're viewing: `<truck>` — `<van>`" always shown → VAN switcher (only when the truck has >1 van; truck-scoped) → TRUCK switcher (only when >1 PERMITTED truck; auto-hidden/silent for single-truck users) → DEFAULT SCREEN → NOTIFICATIONS → APP-LOCK toggle → per-device subtext. Truck switch → full reload to the new token → re-scopes console + push. Shown in BOTH the dashboard `UserMenu` and the KDS header entry.
- SCOPE NOTE (consistent with the backlogged security gap): this gate is TRUCK-level (enforced server-side — a user can only switch to trucks they own/are a member of). It is NOT van-level — a multi-van truck's member can switch the device to any of that truck's vans, because van-scoping (`truck_user_vans`) is the soft, unenforced hint flagged in the V8.6 security backlog. Truck-level enforcement is correct and real; van-level remains the backlog item. Fine while all trucks are single-van.
- No new migration — `van_devices.truck_id` already exists (NOT NULL); the switch is an UPDATE. bind-device (van/screen/notify WITHIN the current truck) stays token-authed; the cross-truck switch is the session-authed endpoint.

**Biometric / passcode app-lock (built; hardware-gated for real behaviour):**
- Optional, OFF by default, PER-DEVICE (`lib/native/appLock.ts`, per-device localStorage). Toggle in `ThisDeviceSettings` (verifies once on enable). Dep `@aparajita/capacitor-biometric-auth` (^10).
- `AppLockGate.tsx`: full-screen overlay prompting Face ID/Touch ID + passcode fallback on launch AND every foreground (via `onAppResume`). Unlock reveals the console — NOT a re-login (session stays). Rendered in dashboard + KDS.
- Device-level security (someone picks up the unlocked iPad), SEPARATE from authentication. Real biometric behaviour is hardware-gated (simulator can fake a match). Needs `NSFaceIDUsageDescription` in Info.plist + the biometric pod + cap sync.

**Apple ID login:** BACKLOGGED (login is rare given persistent sessions → low value vs cost; mandatory only if social logins like Google/Facebook are added later).

**Simulator-testable now:** login + persistence, native landing routing, truck/van switch, membership gate. **Hardware-gated:** real Face ID/passcode, APNs (send/registration/tap), cold-launch routing, real network detection, and the WKWebView-session-survives-hard-kill confirmation. **On the Mac when hardware arrives:** `npx cap sync ios`; the biometric + push pods; `NSFaceIDUsageDescription`; the Push Notifications entitlement; `APNS_*` env; the paid Apple Developer account for TestFlight + the 4.2 review.

### Migrations — run by hand (additive; deploy-coupled to /api/native/* + the submit push path)

- `20260701_van_devices.sql` — CREATE TABLE van_devices (id, truck_id→trucks ON DELETE CASCADE, van_id→truck_vans ON DELETE SET NULL, device_id text UNIQUE, push_token, platform, default_screen CHECK dashboard|kds default 'dashboard', notify_enabled default true, last_seen, created_at) + van_idx + truck_idx. NOTE: `van_id ON DELETE SET NULL` → deleting a van silently UNBINDS its devices from notifications (they need re-binding); acceptable default, flagged behaviour.
- `20260701_van_notification_prefs.sql` — CREATE TABLE (van_id→truck_vans ON DELETE CASCADE, type, enabled default true, PK(van_id,type)).
- Run BOTH before deploying the app code (the submit trigger try/catch degrades gracefully if absent — order still saves, push no-ops — so not a hard deploy-break, but the feature won't work first-request without them). Verify via `information_schema` (read rows, not the success flag).

### Pending migration state (full)

DONE (this + prior session): `hide_name` (deploy-coupled), `lactose_allergen_to_dietary`, `allergen_audit_card_match` (widened CHECK to include 'card_match' — verified via `pg_get_constraintdef`), `preorder_open_rule` (verified: all trucks 'on_confirm', no nulls). PENDING: `20260701_van_devices`, `20260701_van_notification_prefs` (both additive, deploy-coupled to /api/native/*). *(**⚠️ CORRECTION — V9.2:** BOTH ARE **APPLIED**, verified against the live schema 27 July 2026. This entry recorded them as pending and they had in fact run — **recorded-as-pending, actually-applied**, the exact INVERSE of the §65 incident and the same class: a belief about the database is not a fact about the database. The inverse direction is the more dangerous one, because it invites someone to "finally apply" a migration that already ran, or to rewrite the file expecting it to land. See §16.)*
...
- **`handleCommitMenu` missing else/finally + swallowed `{ok:false}` → silent infinite spinner on ANY commit failure (HIGH — carried, still open).** Durable fix: `finally` clears spinner + toast on non-ok.
- **`truck_user_vans` van-scoping is a SOFT login-redirect hint, NOT an enforced access boundary (SECURITY — new).** `/api/dashboard` serves truck-wide data regardless of member van-scope; a scoped staffer navigating to the full dashboard URL gets ALL vans' data. Enforcing it needs reliable per-member identity, which is cookie-dependent and absent in the native shell (role falls back to owner). NOT a live exposure while all trucks are single-van. Fix required BEFORE onboarding a multi-van truck that needs staff van-restriction. DO NOT claim per-van staff restriction as a feature until enforced.
- Kitchen-setup step: show + persist prep for PRE-EXISTING categories (specced V8.5, not built).
- Per-option allergen verified gate (close the §17 gap when the per-option review feature is built; deploy-coupled + Gusto-touching; sets Gusto's real option allergens verified=true at deploy).
- Dead-code sweep: orphaned card-mode chip flow.
- Free-form allergen NOTE in per-dish mode (per-dish chips PLUS optional free-form note; reuses `allergen_info_text` + verbatim editor).
- Systematic AbortController pass on external fetches (geocode fixed; other untimed fetches remain a latent hang vector).
- iPad app Phase 2 (background connectivity best-effort — reduce false pauses; NOT a guarantee) + all hardware-gated Package 1/5 validation.
- Van-delete → device re-bind prompt (currently SET NULL silently unbinds).
- RTF handover: gated on the live-batch deployed-build re-test; RTF will check everything before placing customer orders.
...
**§65 — Extras stock: per-event leak fix + decrement-pool → CEILING conversion. [BUILT; TS + ceiling engine LIVE, but the `place_order_atomic` SQL rewrite was NOT applied until 2026-07-15 — see the CORRECTION below].** Options now use the SAME ceiling-vs-live-tally model as menu items — the decrementing pool is gone.

**🔴 CORRECTION (V8.8, 2026-07-15): the "[APPLIED/live]" claim was FALSE — this is the record that hid a broken production path for weeks.** The §65 `place_order_atomic` SQL rewrite (drop `p_draw_list` + the option-draw loop) and the `DROP` of `decrement_/increment_modifier_option_stock` were **never committed to `supabase/migrations/` and never applied to production.** The TS shipped calling the 7-param signature (no `p_draw_list`); prod kept only the OLD 8-param function → **every customer `/api/orders/submit` failed with `PGRST202` ("Could not find the function public.place_order_atomic(…) in the schema cache")** from the V8.0 deploy until 2026-07-15. It was MASKED because **operator orders use a DIRECT `orders` insert (`dashboard/action`), never this RPC** — operator-only usage hid a fully-broken customer path; found only when a customer test order was placed on test-truck (Gusto's night-1 was operator-add only, so no real customer order was lost — it would have failed for every customer the moment online ordering turned on, which it did 15 Jul). FIXED + COMMITTED: `supabase/migrations/20260715_place_order_atomic_drop_drawlist.sql` — the LIVE def was dumped (`select pg_get_functiondef(oid) from pg_proc where proname='place_order_atomic'`) and confirmed un-drifted, so the new body = the committed `20260624` original **minus exactly** the `p_draw_list` param + the draw loop + the two dead declares (a DELETION, not a rewrite — order-number logic, all 19 INSERT columns/values, the `production_slot_usage` DELETE-then-INSERT, `RETURNING order_key`, and the return shape are byte-identical); then dropped the 8-param overload + the orphaned helpers + `notify pgrst, 'reload schema'`. Verified by reading `pg_proc` (exactly one `(jsonb,text,text,uuid,text,date,jsonb)` row; zero helper rows) + a live customer test order. **LESSON: "recorded as applied ≠ applied" — verify hand-run migrations against the DB and commit every one; deploy-coupling applies to function SIGNATURES too. See the V8.8 do-not-relearn cluster (item 1).**
...
> ✅ **DEPLOYMENT STATUS (V8.0 + V8.1) — DEPLOYED / LIVE.** The whole batch is committed + pushed (Vercel auto-deployed to production, incl. **Pizzeria Gusto**). All DB changes are **APPLIED**: the V7.9 set (`20260624`, `20260625`), `event_option_stock`, the order-ready columns (`truck_vans.order_ready_enabled`, `truck_events.order_ready_override`), the 4 scrape columns (§61), the truck-email column (§67). **🔴 CORRECTION (V8.8): the `place_order_atomic` rewrite + the `DROP` of `decrement_/increment_modifier_option_stock` (§65) were asserted here as "now live" but were NEVER applied until 2026-07-15 (`20260715…`) — see the §65 CORRECTION. Every customer order failed with PGRST202 in the interim.** The formerly-untracked files (`lib/useToasts.ts`, `lib/useReadyEmailUndo.ts`, `components/ToastStack.tsx`, `lib/copy/stock.ts`, `.github/workflows/hatchgrab_scrape.yml`) are **tracked**. The historical pre-deploy notes below are retained for context but no longer action items.
>
...
**§61 — Per-truck scrape cadence (background infrastructure; NO control UI). [BUILT + LIVE; CORRECTED V8.8].**

**⚠️ CORRECTION (V8.8, 2026-07-15): the V8.0 "fixed local-timezone slots" model that earlier versions of this section described — `isScrapeSlotNow` / `SCRAPE_SLOT_HOURS = [8,14,20]` / `dueByFrequency` / cron `0 7,8,13,14,19,20` — was REMOVED. It caused the 06-28 regression (below). This section now describes the CURRENT due-window model. Do NOT trust older descriptions of exact-hour slots — that mechanism no longer exists.**

Each HatchGrab-linked truck (`trucks` row with `scraper_preference IN ('auto','both')` + a `schedule_url`) is re-scraped on its OWN cadence in the Pass-B loop (`scripts/run-scraper.js`).

- **THE 06-28 REGRESSION (why the exact-hour model is gone):** commit `2e94c04` ("Major upgrade") added an `isScrapeSlotNow` gate requiring the London hour to be EXACTLY 8/14/20, on cron `0 7,8,13,14,19,20`. But **GitHub Actions cron is routinely delayed** (PROVEN: a `0 6` discovery job fired at 09:24 UTC, ~3h24m late) → runs almost never landed on the exact hour → **the whole run scraped nothing.** Structurally, in BST only the 07/13/19 UTC firings could ever map to a valid local hour (08/14/20 UTC → 09/15/21 local = never valid). A gate-rejected truck hits `continue` BEFORE `recordRunAndLearn`, so it writes **no `scraper_run_log` row** → "zero rows since 06-28" meant "passed the gate zero times," not "ran and found nothing" (test-truck: 30 logged runs 06-21→06-28, then nothing; gusto: only off-cron manual dispatches, which bypass the gate).
- **CURRENT MODEL (due-window + hourly cron):** gates = `shouldRunToday(truck) && isDueByLog(truck)` — the exact-hour gate is REMOVED. Due = a forgiving **due-WINDOW**: `dueWindowHours = max(1, 24 / scrape_times_per_day − 1)` (3×/day → 7h), computed from the **authoritative `scraper_run_log`** (NOT `trucks.scraper_last_run_at`). Cron → **`0 * * * *`** (hourly — absorbs CI jitter, timezone-agnostic/duration-based, zero per-zone maintenance), with an `anyDue` pre-check that exits before launching Chrome. `SCRAPE_TRUCK_ID` manual dispatch is unchanged (bypasses the gate). SQL run by hand 07-14: `UPDATE trucks SET scrape_times_per_day = 3 WHERE scraper_preference IN ('auto','both') AND schedule_url IS NOT NULL`.
- **⚠️ `trucks.scraper_last_run_at` is UNRELIABLE — the test-truck cleanup NULLs it. `scraper_run_log` is the authoritative run history.** (This one fact would have prevented the first wrong diagnosis this session: "test-truck never ran," when it had 30 logged runs.)
- **`shouldRunToday` (learned-DAY gate, unchanged):** `!scraper_learning_complete` → true every day; once learned (30+ days of change-history → `scraper_update_day`), scrapes on that weekday ±2.
- **Frequency — new=3 / existing=1:** `trucks.scrape_times_per_day int DEFAULT 3` (new trucks 3×/day via the column DEFAULT — no in-app creation insert to hook), one-time backfill existing → 1. Adjusted by a rare manual DB edit — deliberately NO control UI.
- **Two-job cron (`SCRAPE_MODE`):** `daily_scrape.yml` (`cron 0 6 * * *`, `SCRAPE_MODE=discovery`) = Pass A (global discovery), 1×/day; `hatchgrab_scrape.yml` (`SCRAPE_MODE=hatchgrab`, now **`0 * * * *`**) = Pass B; **unset → both** (local + `workflow_dispatch` stay whole-scrape). Pass A is never tripled by the Pass-B cron.
- **Text-hash cost-skip:** a PRE-Gemini guard hashes the raw page text (`hashText(winningText)` vs `trucks.scraper_last_text_hash`); byte-identical → skip the Gemini extract + inbound POST (Puppeteer-only), stamp last-run. Stored ONLY after a successful extract, so a failed extract retries, not wrongly cached.
- **Safety at 3×:** re-scraping is idempotent — `inbound-schedule` dedups via `scraped_signature` + reject-memory. `scraper_run_log` grows ~3× for frequent trucks but `pruneScraperRunLog` bounds it to 90 days. Migrations (`scrape_times_per_day` + backfill, `scraper_last_run_at`, `scraper_last_text_hash`, `timezone`) are APPLIED/additive; `timezone` is retained but the current due-window model no longer keys cadence on local hour.

**`scraper_rule` SEMANTICS (were NEVER documented — the gap that caused two wrong diagnoses this session):**
- **`scroll_lazy`** = `performModernScroll` — a schedule on ONE lazy-loading page: deep-scrolls (150px steps, up to 1.5× scrollHeight / 15s) then dumps `innerText`.
- **`scroll_next`** = `performButtonHunt` — a schedule across MULTIPLE pages: clicks "next/load more" ≤4×, appends each as "PAGE N". **Does NOT deep-scroll → UNDER-READS a single lazy-load page** (misses below-the-fold). Do NOT use for a lazy-load page.
- Two more exist in code but are NOT wired to `scraper_rule`: `performExpandAndScrape` (accordion), `performFrameDump` (iframe). Pass-B branches `scroll_next` → buttonHunt, everything else → modernScroll. `verify-schedule-url` has byte-mirrored copies and dual-runs both.
- **Winner = dual-detection by DATE-PATTERN COUNT, not hit-count "reliability."** WORKED EXAMPLE: Gusto's steady `scroll_next` "2–3 events/run" LOOKED reliable but was a CONSISTENT UNDER-READ (buttonHunt never deep-scrolls a single lazy page); `scroll_lazy`'s "flaky" 5/2/0 was CORRECT — it found the 4–5 events `scroll_next` was missing (0 = genuinely no future events). **LESSON: a consistent under-read can look "reliable" while being wrong for the page structure — judge by PAGE STRUCTURE, not hit count.**
- **Zero-result reset caveat:** a legitimately 0-event day trips the SAME rule-reset as "wrong rule" (`:1278-1287`), wiping `scraper_rule` to NULL; it self-re-detects next run → **a NULL rule is often just "had a 0-event day," not a fault.**
- **Gusto = single lazy-load page (`pizzeriagusto.co.uk`) → `scroll_lazy` is CORRECT. The self-heal flip `scroll_next`→`scroll_lazy` was a CORRECTION, not degradation. Do NOT pin `scroll_next`.**
...
Sticky layout contract (SUPERSEDED by the app-shell — V8.7 iPad-native): the dashboard and manage pages no longer stack `position:sticky` chrome against body scroll (that pattern was unreliable in the iPad Capacitor WKWebView — the tab bar detached and scrolled away mid-content). Both pages are now a **KDS flex app-shell**: outer `h-dvh flex flex-col overflow-hidden`, with AppHeader + tabs (+ dashboard event bar) as **non-scrolling `shrink-0` flex children** and a single `<main flex-1 min-h-0 overflow-y-auto>` as the ONLY scroll container. The chrome is physically outside the scroller, so the old hardcoded top offsets (top-[51px]/top-[95px]) are GONE — do not reintroduce them. AppHeader keeps its own `sticky top-0 z-50` (shared with other pages; inert-but-harmless inside the shell). z-order top→bottom: header 50 > tabs 40 > event bar 30 > content. Any new operator page must reuse AppHeader + slate-900 tabs inside this shell rather than re-deriving offsets.

> **Sticky headers INSIDE `<main>` pin to `top-0`, and the scroller must have NO top padding.** Because `<main>` is now the scroll container, a `sticky top-0` child (e.g. the Billing plan/price header, the orders Day-load sidebar) pins at the top of `<main>` — right under the fixed tabs. CRITICAL: a sticky element pins at the scroll container's CONTENT box, so any `padding-top` on `<main>` becomes a permanent gap above the sticky header (rows show through). The manage `<main>` therefore has NO top padding (`px-4 pb-6`); the resting top gap lives on an inner `<div className="pt-6">` as SCROLLABLE content. Never put the top gap on the scroll container when it holds a sticky-top child.
...
### Live-schema facts — demo, deletion order + printing (V9.2 / 28 July 2026)

- `demo_sessions.extraction_source` — `text`, nullable. `'upload'` = the visitor's own menu, `'template'` = a sample they picked. **Nullable is required, not incidental:** the session row is created at provision time and the source is written only after the menu resolves, and a failed extraction never writes it at all. Signup (`/api/setup` GET) re-commits an `'upload'` payload onto the real truck and **ignores a `'template'` one**, so a sample menu can never land on an operator truck. Migration `20260728_demo_sessions_extraction_source.sql` — written as `add column if not exists` because the column was already hand-applied to production.
- **No CHECK constraint on `extraction_source`**, deliberately: the vocabulary is enforced at the single call site by a TypeScript parameter type, and a constraint added now would have to validate rows written before it existed. If a third source is ever added, the value goes in first and the constraint second.
...
## Notification preferences — `device_notification_prefs` (V9.2, APPLIED)

**One store, one axis, for all three alert types.** Preferences had drifted across three stores on three different axes, by accident rather than design: `van_notification_prefs` (per-VAN, so two devices on one van cannot differ), `van_devices.notify_enabled` (per-device but ONE boolean for everything), and device-local Capacitor Preferences (`hg_notify_master` / `hg_notify_offline` / `hg_notify_neworder` — per-device AND per-type, but **invisible to the server, so unable to gate a push the server sends**). The agreed model is per-device preferences for every alert type, which only the last axis could express and only the first two were visible to.

- **`20260728_device_notification_prefs.sql` — APPLIED (27 July).** `device_notification_prefs(device_id text → van_devices(device_id) ON DELETE CASCADE, type text, enabled boolean NOT NULL DEFAULT true, updated_at, PRIMARY KEY (device_id, type))` + a CHECK constraining `type` to `'offline_protection' | 'order_pending' | 'schedule_received'`. RLS on, **zero policies** (service-role only, same posture as `van_devices`). **No separate index on `device_id`** — the PK's leading column already serves "all prefs for this device", which is the only read shape either the client or the send paths need.
  - ⚠️ **The FK targets `van_devices.device_id` (text, UNIQUE), NOT the uuid PK** — a deliberate divergence from the usual uuid-FK rule. `device_id` is the key every caller already holds (the client generates it; `/api/native/bind-device` upserts `onConflict: 'device_id'`), and both are stable across re-binding because bind-device UPDATEs the row rather than replacing it.
  - **A MISSING ROW READS AS ENABLED.** Same convention as `preorders_enabled !== false`. For an *alert*, the safe direction is to fire: a missed offline-protection alert means orders arriving at a device nobody is watching, which is the exact failure the alert exists to prevent.
- **`20260728_device_notification_prefs_backfill.sql` — APPLIED (27 July).** Existing devices get EXPLICIT rows so nothing changes behaviour on deploy. `order_pending` folds today's two server-side gates into one row (`coalesce(notify_enabled, true) AND coalesce(van_notification_prefs.enabled, true)`), so a muted van becomes a set of muted devices. `offline_protection` and `schedule_received` seed **false** — the former because `hg_notify_master` is unset out of the box (so those alerts do not fire today for anyone who never opened Settings), the latter because it is brand new and the scraper bridge runs hourly.
  - **VERIFIED STATE: 27 rows across 9 devices** — `offline_protection` **0 enabled / 9 disabled**, `order_pending` **8 enabled / 1 disabled**, `schedule_received` **0 enabled / 9 disabled**. The verification that mattered was not the counts but a diff: recomputing the pre-migration gate arithmetic from the old stores and comparing it to what landed returned **zero rows**, proving every device's new pref reproduces its old behaviour exactly.
- **`20260728_notification_prefs_retire_old_stores.sql` — WRITTEN, DELIBERATELY NOT RUN.** It `DROP`s `van_notification_prefs` and drops `van_devices.notify_enabled`. **`app/api/orders/submit/route.ts` still reads BOTH**, so running it today means every customer order on a live trading truck hits a missing table and a missing column. Preconditions, all required: the new table populated (met); the submit path reading it and no longer referencing the old two; the Settings card writing it; **the one-time client migration of the local `hg_notify_*` keys shipped and run**; that code DEPLOYED, not merely committed; a real order observed post-deploy; and a clean `grep` for the old names outside comments. **Only the first is met.**

> **BOTH OLD STORES ARE LEFT DORMANT, NOT DROPPED** — the manual's established leave-dormant-then-sweep precedent. Keeping them intact through the switchover makes the rollback for the whole rework a **code revert**: no data destroyed, the old readers still find what they expect, the new table simply ignored. Dropping them in the same sitting as the create would make the switchover irreversible on its riskiest day. Two dormant objects cost nothing; an unrollbackable deploy costs a trading day.

> ⚠️ **NOT SWEEPABLE FROM SQL:** the device-local `hg_notify_*` Capacitor Preferences keys live in each device's localStorage. They are retired by the client code that stops reading them — which is why that one-time migration is a precondition above and a **blocker** in §27, not a nicety.
...
### Found, reported, not fixed
- **The `online_paused_until` relocation left stale names behind.** It moved from `truck_vans` to `truck_events`, and the surrounding names and comments were never revisited: `/api/dashboard` returns a key named **`vanOnlinePausedUntil` carrying an event-scoped value** (the route's own comment admits the key was kept for client compatibility), the client state inherits the misleading name, and a comment at `page.tsx:347` still claims `truck_vans.paused_until`. ⚠️ **Whether the MANUAL `paused_until` also moved was not determined.** A rename pass would settle all of it and it needs its own session — renaming a field that a client state variable inherits is not an end-of-session change.
- **`whatsappStats` is computed server-side, returned, typed on the client — and never rendered.** Four references, zero JSX. It is fetched and discarded on every report load, an unscoped `whatsapp_logs` query for nothing. ⚠️ If a panel is ever added, it **must be hidden under a van filter**: `whatsapp_logs` has no van column, so a whole-truck number would sit beside filtered ones inviting an invalid comparison, and hiding beats labelling because a label asking the reader to remember that is a label they will not read.
- **`InlinePriceEditor` does `useState(price.toFixed(2))` and never re-syncs** — if the order's price changes server-side while the card is mounted, the editor opens on the old figure. ⚠️ There is a real tension: re-syncing mid-type would clobber the operator. **The right fix is seeding on ENTERING edit rather than at mount**, which is a small deliberate change, not a sweep.
- **A non-active selected event in the Add Order panel can still resolve stale settings.** `upcomingEvents` inside that panel is local state fetched once, not a prop, so there is no live source for an event other than the dashboard's active one. It does not affect any reported bug. **The fix, if ever needed, is to pass the parent's array down — not another sync effect.**

### Deferred by decision
- **Section headings over groups of Settings cards** (option B). Real information architecture and it would scale, but it adds height to a tab that already scrolls, and a taxonomy designed at seven cards usually needs redoing at twelve. **Do A-then-B if the card count keeps growing** — they are compatible. ⚠️ If it happens, `SUBCARD_HEADING` would become the *section* token and Manage's sub-card headings a different thing again: keep them distinct rather than letting one token serve two jobs.
- **Instant cross-device van pause.** Blocked on moving `last_heartbeat_at` off `truck_vans`. See the invariant; ≤60s via the poll is accepted.

### Unexercised — built but never seen or run
- **The whole V9.5–V9.7 UI batch is unrendered.** Both Settings tabs restructured, the payment card retitled, `Paid & collected`, `💷 Cash` / `💳 Card`, the remove-payment modal, the van filter dropdown and scope chip, the Reports re-key.
- ⚠️ **The check that matters most from V9.7:** put the payment card and the auto-accept card side by side and confirm the first-row top spacing and the divider weight are indistinguishable. That is the entire point of the retitle and the only part not verifiable from code.
- **The paid step appearing in Add Order with NO refresh, and NO second `/api/dashboard` request in the Network tab** — that proves the response-patch path ran rather than the fallback.
- **The basket surviving a settings toggle.** `liveEvent` provably sets no state, but that is the guarantee the whole private-copy design exists to protect.
- **The event banner and Start/Restart button after an open/close** — where the removed `status` effect would show if that removal was wrong.
- **A two-event date in Reports**, confirming each order now shows its own venue.
- **Gusto's sound seed on their iPad** — still the one promise verified only by reasoning.

### Blocked, not by code
- **Stripe Connect: company setup is ON HOLD pending outside factors.** Connect asks for a payout account tied to a business entity. Everything else in the payments plan proceeds without it — the ledger, the paid step, refund copy, balance display. **Only Connect is gated.**

## 🔴 V9.6 — added 30 July 2026

### Found, classified, not fixed
- **🔴 33 further dropped-error destructures in `app/api/dashboard/action/route.ts`** — `{ data: order }`, `{ data: eventRow }`, `{ data: cur }`, `{ data: cats }`, the five-way `Promise.all` at `:1223`, and others. Many are `select('*')` (immune to `42703`) and most are followed by an `if (!x) return` that turns a null into a real response, so **they are not all silent** — but which are genuinely silent versus adequately guarded has NOT been audited. ⚠️ **Do not treat that sentence as clearance.** Needs a dedicated **classify-first** pass: each site by *named-vs-`*`* and by *whether a null is caught downstream*, then log the ones that fall through. This is the write path that moves money, so it is a read-and-classify job, not a sweep-and-edit one.
- **Other routes are unswept** — `/api/manage`, `/api/orders/*`, `/api/events/*`. On this evidence, unlikely to be clean.
- **The prep board aggregates by a constructed display string** — `app/dashboard/[token]/page.tsx:2330` and `:2391`: `` const label = `${name}${parts.length ? ` (${parts.join(', ')})` : ''}` `` used as a map key. Two items whose rendered text collides merge into one row, and changing the modifier-join format silently re-keys the whole map. **This is a live instance of the label-as-key class.** It has not bitten because the format has not changed since it was written.

### Not built, decided
- **Retire `trucks.sound_config`, `set_sound_config` and the `update_settings` allowlist entry.** Precondition: every device has loaded at least once since the V9.6 deploy, or a device that has not would seed from nothing and get the hardcoded default instead of the truck's real settings. `set_sound_config` is already fully dead code, retained deliberately.
- **The keep-screen-on toggle branches on the LOCK, not the INTENT** — `toggleKeepScreenOn` tests `screenHeld` rather than `keepScreenOn`, so after any web navigation (where `prepareKeepAwake()` deliberately sets `wakeState = 'off'` awaiting a user gesture) the toggle reads grey "Screen off" and tapping it runs the **ON** branch. **There is no state in which the operator can set the preference OFF while the lock is not held.** One-line fix: branch on intent, keep the display on reality. Web-specific. ⚠️ Two secondary paths also set the preference on: `toggleOfflineProtection` silently enables it, and the localStorage write is a swallowed `try{}catch{}` so a failed write means "off" never persists.

### Unexercised — built but never seen or run
- **The entire V9.5/V9.6 UI batch has never been rendered.** Both Settings tabs restructured, the regrouped payment card, `Paid & collected` on the order card, `💷 Cash` / `💳 Card`, the remove-payment modal, the reworded copy throughout. ⚠️ **The tightest is `Paid & collected` at the 240px KDS column** — 11px of margin on an *estimated* character advance.
- **Whether `💷` and `💳` are instantly distinguishable at 14px on a 240px column** — that is the entire point of using icons rather than colour, and it is the thing least assertable from code.
- **Whether four stacked grey sub-panels in one Settings card read as organised or as busy.**
- **Gusto's sound seed** — one dashboard load on their iPad, then confirm `hg_soundcfg_<token>` reads `{"new_orders":"all","order_due":true}`. **The one promise this batch made that is verified only by reasoning.**
- **The terminal-failure flow** — no event has ever had `takes_cash_override` written, and the self-expiry is proven by the resolver's arithmetic, not by rolling onto a new event.
- **Neither server-side override read has run** against an event carrying either override.
- **None of the nine new consequence-naming log messages has ever printed.**

### Blocked, not by code
- **Stripe Connect requires company setup.** A sandbox account exists under Village Foodie but Connect asks for a payout account tied to a business entity. **Incorporation and business banking have lead times outside your control — start them early.** Everything else in the payments plan proceeds without it: the ledger, the paid step, refund copy, balance display. Only Connect is gated.

## 🔴 V9.5 — added 30 July 2026 (payments phase 1a/1b)
...
- The over-capacity confirm modal has **never been rendered**.
- The `❗` inside a native `<select>` `<option>` is **unverified on a real iPad**.
- The per-button pending state on the **offline early-return** path is untested.
- The remove-payment modal has **never been opened**; no dashboard modal has a focus trap or `Escape` handler.
- **KDS notes/button spacing** was raised from 8px to 12px in window mode; 16px was recommended and not applied. A Square KDS operator complaint records accidentally completing orders while trying to read a note — this codebase has the same layout.
...
- ✅ **DONE (28 July)** — `demo: { extraction_source, email, expires_at }` is served on `/api/dashboard` for demo trucks only; `?welcome=sample` retired with it. `email` and `expires_at` are exposed but **not yet consumed**.
- Scheduler prior-day sweep should honour `auto_close` or exclude `demo-` trucks — a global cron on a live operator path, own diff.
- ✅ **DONE (28 July)** — the demo event roll's failure modes are resolved by **deleting the roll entirely**; an elapsed demo now ends and offers "Start a new service", and the customer order page shows an honest closed state (onboarding spec §10 Phase 2E).
...
- 🔴 **iOS BILLING / APP STORE COMPLIANCE — DECISION OPEN, and it GOVERNS the other items here.** Apple restricts in-app links to external purchase mechanisms outside the US storefront; HatchGrab is UK, so the **May-2025 US carve-out does NOT apply**. Three options weighed: **(a) keep manage in-app and hide the upgrade/billing UI on iOS at runtime** — one platform check, one surface, no link and no signpost to one; web and Android untouched — **CURRENT PREFERRED**; **(b) remove manage, link out to the web portal — REJECTED as a compliance answer**, because the link *is* the restricted mechanism and making it the only path makes it MORE prominent, not less (it also costs functionality); **(c) remove manage entirely — compliant but a real degradation**, since operators commonly run on ONE device.
  - ⚠️ **THE UNVERIFIED QUESTION THAT DECIDES ALL THREE: does HatchGrab incur an IAP obligation at all?** It is B2B operator software; the subscription buys a truck the ability to run its business, a service consumed **outside** the app, and Apple carves out outside-app delivery with separate allowances for multiplatform and business services. **If that reading holds, the answer is "change nothing".** NOT VERIFIED — needs the **current text of 3.1.1, 3.1.3(b) and 3.1.3(e) read directly from Apple**, and is worth professional compliance advice before submission. **Do not restructure the app on an unverified premise:** (a) is cheap and reversible; (b) and (c) both destroy functionality to satisfy a rule we have not confirmed applies.
...
**Porting a permission bypass without the routing branch that CONSTRAINED it widens access silently.** A bypass is only ever half a rule; the other half is wherever the product decided what to do with it. *Evidence:* `/api/native/my-trucks` inherited the web admin bypass, but `/app` never got the web router's `admin → /admin` redirect — so an admin landed on an arbitrary truck's dashboard instead of the admin console. Nothing errored; the permission was correct and the destination was not. **When copying an authorisation branch to a new surface, copy its ROUTING too, or state why the new surface doesn't need it.** **SWEEP: OPEN — the native surface was fixed; no audit has been done of other web→native permission ports.**

**An unordered query feeding a first-row pick is NON-DETERMINISTIC, not merely wrong.** Postgres may return rows in any order without `ORDER BY`, so `rows[0]` can differ between two identical requests — which makes the bug intermittent and unreproducible rather than simply incorrect. *Evidence:* `/api/native/my-trucks` had no `ORDER BY`; `trucks[0]` — the truck the app opens on — could vary per request. **Same class as V7.8 §3.** **SWEEP: CLOSED — all three `app/api/native/*` routes checked: `my-trucks` (`:49`, `:79`) and `switch-truck` (`:39`) now `.order('created_at', { ascending: true })`, matching the web router exactly; `bind-device:36` picks `vans[0]` only under `vans.length === 1`, so it is deterministic by construction.**

**Native tooling enforces filename and content constraints the web codebase never does.** Two confirmed, both build-breaking and both invisible to `tsc`, ESLint and code review: **an XML comment cannot contain `--`** (SAXParseException at resource merging, which fails the Gradle build before anything else runs), and **Android resource filenames must be lowercase `a-z`, `0-9` or underscore only**. **When adding files to `android/` or `ios/`, the constraints are the PLATFORM's, not the repo's.** **SWEEP: CLOSED — both native trees clean; the double-hyphen rule is recorded at the top of `android/app/src/main/res/values/styles.xml`, where it was hit.**

**A swallowed write makes a missing column indistinguishable from a working one.** `demo_sessions.extraction_source` was written and read by code and present in production, but in no migration file. It went missing because it is written in the same `update` as `extraction`, whose three siblings all got a phase-4 migration — and the write is best-effort, logging `console.warn('… (migration applied?)')` and continuing. So a missing column would have looked exactly like a working system, right up until signup silently fell back to a blank upload. **That something works in production is not evidence that a migration exists** — and where a write is deliberately best-effort, its `console.warn` is the only signal this class of fault will ever produce.

**A DOM API scoped to the wrong element fails silently.** `scrollend` was listened for on `window`, where it never fires: the board scrolls inside `<main overflow-y-auto>`, not the document. No error, no warning — the handler simply never ran. Same family as `next/image` shadowing the global `Image`: an API that looks global but is scoped to something else. Where a scroll container is not the window, the listener belongs on that container (or capturing on `document`).

**Renaming a string-keyed map entry can silently disable a guard.** `ROW_FEATURE_MAP` is keyed on the compare table's row NAME, and feeds `findPlanParityViolations()`, which cross-checks the marketing table against the enforcement gate in `lib/features.ts`. Rows absent from the map are skipped as "marketing-only". So renaming the row without renaming the KEY would have dropped the KDS row from the guard — **it would have reported clean while no longer checking anything.** Type-checking cannot see string-keyed lookups; when renaming anything that a map keys on, grep for the old string.
...
**Two different outcomes must never share one return shape.** `recordCollectionPayment` returned `{inserted: false, chargedMinor: 0}` for both "this event is already recorded" (a correct replay) and "there was nothing outstanding to charge" (a no-op). No caller branched on the flag, so a swallowed money-event returned 200, the operator saw success, and the audit log recorded a successful action that moved no money. **The information was present and structurally unusable.** When two paths can return the same shape and only one of them is an error, either the shapes must differ or something must detect the difference — see the detector invariant below.

**A DETECTOR beats a scheme when the scheme cannot be complete.** No deterministic, server-derived idempotency key can be collision-free: if the key is a function of ledger state, any sequence that returns the ledger to an earlier state and repeats the same transition must collide. Rather than iterate on key formats, the fix is a **state-change assertion** — a swallowed duplicate where the balance FELL is a genuine replay (silent, correct); one where the balance is UNCHANGED is a real collision (throw, surface). **This holds for any residual case, including ones nobody has thought of**, and it correctly stays silent on the concurrent race the unique index exists for, because the second request's recalc reads the ledger after the first request's insert. **Where a uniqueness scheme cannot be proven complete, assert on the EFFECT instead of refining the key.**

**A warning comment is not a control.** The projection-omission class was documented in three places, swept and closed in V9.0, and carried an explicit note that "a newly-omitted field reopens the class" — sitting four lines above where `show_paid_step` was then omitted. **The durable fix was to invert the failure mode, not to warn harder.** Spread-and-redact means forgetting a field leaks it (visible, harmless) rather than omitting it (silent, breaks a feature). **When a class keeps recurring despite documentation, the mechanism is wrong, not the documentation** — change which direction the mistake fails in.

**PostgREST `UPDATE` writes only the keys present in the object.** A "whole-object save" therefore CANNOT re-default a column it does not name, and "send only changed fields" is already true throughout this codebase. This was assumed to be the cause of a reverting setting and was not. Recorded so it is not re-chased.

**`trucks` has NO `updated_at` column** — only `created_at`. Meanwhile `orders` has **two** triggers stamping one `updated_at`. Both are the same species: the schema and everyone's assumption about it disagreeing, with nothing checking. **Before using a timestamp to reason about when something changed, confirm the column exists.**

**Attribution must never become authorisation.** Reading a cookie to record WHO acted is a logging concern; it must not introduce a new way for a live operator action to be REFUSED. `resolveActorSafe` cannot throw and cannot return a response — if identity resolution fails, the action proceeds as `actor_kind: 'token'`. ⚠️ And the inverse: wrapping the GET route's resolver in a blanket try/catch would have converted a genuine auth failure into a silent success, letting a foreign operator through during an auth outage. **Two resolvers, deliberately: one that may throw (auth), one that never does (logging).**

**Fail-open and fail-closed are the same policy applied to different failure states.** `collected` fails OPEN on a ledger failure — the order still collects, a warning returns — because blocking an operator at the hatch mid-service is worse than a recoverable gap, and the money physically moved regardless. `undo_collected` fails CLOSED — the audit row is written first and the delete aborts if it fails — because an undo that erases a payment record with no log is precisely the state the audit log exists to prevent. **They look asymmetric and are not: the rule is "never leave the ledger lying", and the two actions fail into different states.** Note also that a collect failing open leaves an *inconsistent but recoverable* state, while an undo failing open would leave `payment_status = 'paid'` on an uncollected order AND leave the idempotency key occupied, silently swallowing the next legitimate collect.

**Do not change UI the operator did not ask about.** If an instruction cannot be satisfied without changing something outside its scope, STOP and say the instruction is contradictory. **Do not resolve it in the direction that expands scope, even flagged** — a prominent flag is not consent. This was broken when "make Ready match the toggles" was resolved by moving the toggles instead, using a *white-text-on-green* contrast ratio to justify changing a toggle track **that carries no text**. Two errors compounding: applying a text threshold to a surface with no text, and resolving a contradictory instruction rather than returning it.

**Contrast is symmetric.** Orange text on white is the same ratio as white text on orange — swapping figure and ground changes nothing. An outline button is not inherently more legible than a solid one; it is legible only if the text colour is dark enough on its own terms. Recorded because this planning chat asserted the opposite while specifying a fix.

**Colour encodes what KIND of action something is, never which variant.** The dashboard vocabulary is **orange = action (confirm, take money), green = ready (a kitchen state), slate = done (clear from board)**, with all three defined once in `lib/ui-tokens.ts` alongside their measured contrast ratios. Cash vs card is a variant, not a kind — it is distinguished by **icon** (`💷` / `💳`), not by two near-identical oranges which would read as a rendering mistake. ⚠️ **Emoji is this codebase's established button-icon vocabulary** (`✓ Confirm`, `✏ Edit`, `↩ Undo`, `🍕 Ready`) and there is no icon library in `package.json` — do not add one for a single button.

**§10's fast-tap rule governs PRIMARY SERVICE actions, not corrections.** No popups when adding items or taking payment — the operator has no time mid-transaction. But removing a payment is a deliberate correction, after the fact, of a record saying money changed hands: **a confirmation step there is appropriate, not friction to be eliminated.** The inline two-tap confirm that preceded the modal clipped its own "Keep" escape off the card edge in a 240px column — losing the cancel affordance from a destructive confirm is a correctness failure, not a cosmetic one.

**🔴 An unapplied migration whose column is named in an explicit `select` returns HTTP 200 with an empty array.** Not an error, not a 500, not a console message, not a failed request. PostgREST rejects the query with `42703`; if the error is destructured away, the result is `null`, every downstream null-guard does its job *correctly* by propagating "no data", and the route returns a well-formed empty response. **And where an empty response is a SUPPORTED state — an empty orders board is documented behaviour when no event is selected — the bug wears legitimate behaviour as a disguise.** This is the fourth member of the recorded-as-applied family and the one with the worst symptom-to-cause distance: it presented as a copy change breaking a status filter, two passes after the actual cause landed.

**`select('*')` DEGRADES; A NAMED SELECT THROWS.** A `select('*')` against a table missing a column you expected returns rows without that field — the consumers see `undefined` and their `??` fallbacks handle it. An explicit `select('a, b, c')` naming a column that does not exist fails the **entire query**. So the deploy hazard is not "a new column" but **"a new column named in an explicit select"**. Before any deploy that adds a column, enumerate every explicit select referencing it — that list is short, and it is the whole risk surface.

**Every query error must be captured, and the log must name the CONSEQUENCE.** "Query failed" tells you nothing an operator can act on; "the board will render EMPTY" does. One sweep of `/api/dashboard` found **five dropped destructures covering nine queries** — including both `orders` queries — while only two captured theirs. ⚠️ **Log, do not 500.** An empty board with a loud log is recoverable: the operator sees something is wrong, can report it, and the next poll self-heals once the cause is fixed. A 500 takes the dashboard down entirely, and an operator mid-service is worse off with a dead page than a visibly empty one.

**A dropped query error plus a `?? default` consumer produces a PLAUSIBLE CONFIGURATION, not a fault.** `truck_vans` is the worst instance: every consumer reads `van?.x ?? default`, so a failed lookup resolves to *"this van has no capacity limit, no cooking step and no order-ready step"* — a legitimate setup, indistinguishable from a real one, on a path that runs every poll. `paidStepFor` has the same shape: a failed event lookup silently ignores every per-event override and falls back to truck defaults. **Where a failure path and a valid configuration are indistinguishable, the log is the only thing that makes the divergence findable.**

**On localhost, the working tree IS the deploy.** Code goes live the moment it is saved, so any migration classified "apply BEFORE deploying" is violated by default during development. This is not a discipline failure, it is a property of the environment — and it is why a correctly-classified, correctly-headered deploy-coupled migration still broke the board. **Apply the migration when you WRITE the code that names the column, not when you deploy it.**

**Nullable-means-inherit, resolved at read time, is the correct override shape — NOT seed-at-creation with a bulk write.** `order_ready_override` bulk-writes every existing event when the truck default changes, wiping overrides the operator set deliberately. That is defensible for pause (a truck changing a pause default probably does mean "everywhere") and wrong for anything the operator sets per-event on purpose. Null-means-inherit gives three properties for free, with no code: changing the default reaches every un-overridden event, overridden events keep their value, and **an override never carries forward — which is also how it self-expires.** A card terminal failing mid-service needs cash enabled tonight and inherited-off tomorrow; the absence of seeding *is* the expiry mechanism.

**`??` not `||` when resolving an override.** An explicit override of `false` against a default of `true` must resolve to `false`. `||` treats `false` as "unset" and silently re-inherits the default — the one substitution that reads as correct and is wrong. Assert it with a test for every boolean override, because nothing else will catch it.

**"Is this a property of the business or of the pitch" is the wrong question when equipment can fail.** `takes_cash` was ruled truck-level-only on exactly that reasoning, which was sound for the case in mind and incomplete: a card terminal failing mid-service is an event-level need for a business-level property. **When scoping a setting, ask what happens when the thing it assumes stops working.**

**A class string moved into a `.ts` file only renders if Tailwind scans that file.** This is the one way a "pure refactor" of a className can silently change output. Tailwind v4 auto-detects sources project-wide and `lib/ui-tokens.ts` already ships working classes, so it is safe here — but the check is cheap and the failure is invisible.

**Record the EXCLUSIONS at the definition site, not just the rule.** When `SUBCARD_HEADING` was extracted, three other labels shared an almost-identical class string and were deliberately left alone — a board column label sits above live tickets an operator scans past and must not compete with them, whereas a settings heading should be as prominent as its rows. Without that note at the definition, the next reader finds three near-matches, assumes the extraction was left half-finished, and "completes" it. **A deliberate divergence that looks like an oversight must say so where it will be found.**

**🔴 `useState(someProp)` copies the prop ONCE. If the component never unmounts, it never re-reads it.** `AddOrderPanel` held a mount-time copy of the selected event, re-synced only when the event **id** changed — so a field flipping on the *same* event was invisible, and the panel never remounts because it is hidden via CSS to preserve the basket. **The tell was that an effect already existed twenty lines below whose only job was patching one field through the same hole; the new bug was the third field to fall through it.** When you find yourself adding a sync effect for a second field, the fix is not a third effect — it is to derive the value on every render from the live source. ⚠️ A sweep found the class is narrower than expected but not empty: a modal that remounts is safe, a value on a 15-second interval self-heals, and one genuine remaining instance sits in an inline editor where re-syncing mid-type would be its own bug. **The sweep covered `components/dashboard/` and the KDS only, and would not match a prop copied via an intermediate variable — treat it as a floor, not a census.**

**Deriving a value beats syncing it, but derive it from the RIGHT source.** The fix above is not "read the parent's prop" — the operator can select a *different* event to take a phone pre-order for, and the payment is recorded against **that** event. The correct shape is **identity from the local selection, values from the freshest source that names the same thing**. Resolving against the parent prop alone would have given a Saturday pre-order Friday's settings. ⚠️ Where no live source exists for the non-active case, say so rather than papering over it — the answer is to pass the live array down as a prop, never to add another sync effect.

**A realtime subscription is only as narrow as its TABLE.** `postgres_changes` filters by row, not by column, so subscribing to catch one field means subscribing to every write on that table. A single high-frequency column poisons the whole table for realtime: `truck_vans.last_heartbeat_at` is written every 15 seconds per device, so publishing that table to watch `paused_until` would make every heartbeat fire a full refetch on every connected client. **Before adding a subscription, enumerate every column on that table and its write frequency.** Same family as one column doing two jobs — a field's write pattern constrains what else the table can be used for.

**A subscription to an unpublished table is silently dead.** The channel is created, the handler is wired, `tsc` is happy, and nothing ever fires. The Supabase realtime publication is the authority, not the code: `select * from pg_publication_tables where pubname = 'supabase_realtime'`. **Check it before writing a subscription and before trusting one that appears to exist** — one had been in the dashboard for months, and its stated purpose had additionally moved to another table without the comment being revisited.

**A column that is inconsistently populated ON PURPOSE cannot be given a second purpose that requires consistency.** `orders.van_id` is a KDS routing field: the customer path sets it, the walk-up path does not, and that asymmetry is what makes it a reliable walk-up discriminator. It is also why **78% of orders carry NULL**, and why a revenue filter keyed on it would have hidden three-quarters of the money while failing to reconcile to the truck total. **Before reusing a column, check its population rate — not its type, not its name, and not whether one write path sets it.**

**`update().select()` is ONE PostgREST statement, and the client should set state from what it returns.** There is no window in which the write succeeds and a separate select fails: `error` unset means the write committed, and a missing row means it matched nothing — **never a failed write, and never to be reported as one.** Setting state from the response cannot refresh the wrong source, because the response *is* the source. ⚠️ Prefer `select('*')` over a named list (a named select naming a missing column fails the whole statement) and avoid `.single()` (it throws on zero rows, turning a no-op into a 500).
...
**`android/**` and the config's `android` block are INERT on web and live on `main`** — the V8.8 rule ("web change → main; native-only can live on main too") applies unchanged. Genuinely native-only files are inert in the browser bundle; putting them on a platform-named branch is what trapped 5,560 lines of web work last time.
