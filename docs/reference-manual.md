HatchGrab Engineering Reference Manual · V8.0
...
Additive on V7.9 (26–27 Jun 2026). Documents this session's features from the FINAL verified code (several changed mid-session — only the END state is documented; superseded V7.9 notes are marked above). New/changed: the **AI menu-import wizard** (§62), the **kitchen-capacity ceiling model** (§63), **order-ready as a MASTER-SWITCH** (§64 — supersedes the V7.9 tri-state AND a mid-session snapshot attempt), the **extras-stock leak fix + decrement-pool→CEILING conversion** (§65 — the V7.9 "approach decided" is now built), the **Ready button + deferred-email + shared toast-undo system** (§66), the **truck-order-email toggle** (§67), and the **default-stock change warning** (§68). Per-truck **scrape frequency** stays at §61 (V7.9, unchanged). Status: **all BUILT + tsc-clean**; the deploy-coupled SQL is flagged per item (see the V8.0 deployment note at the end of this block).
...
**§65 — Extras stock: per-event leak fix + decrement-pool → CEILING conversion (NOW BUILT). [BUILT, tsc-clean; the `place_order_atomic` SQL is DEPLOY-COUPLED — see below].** Options now use the SAME ceiling-vs-live-tally model as menu items — the decrementing pool is gone.
...
- **DEPLOY-COUPLED SQL (pending apply):** the TS `rpc('place_order_atomic', …)` no longer passes `p_draw_list` (`submit/route.ts:823`), so the **new `place_order_atomic` SQL (option-draw loop + `p_draw_list` param removed)** MUST be applied or placement fails on an argument mismatch — apply it WITH/BEFORE deploying this code. Also DROP the now-orphaned `decrement_/increment_modifier_option_stock(uuid,int)` helpers. `event_option_stock` itself is already applied (no value migration — a pool count IS a ceiling number).
...
**§69 — Allergen "not verified" warning system (the launch safety net). [BUILT, tsc-clean].** The interim guard that ships NOW, ahead of the full review wizard: while a truck has unverified allergen data, customers can't see it (it's hidden) and are told to ask staff, and the operator is nudged to verify. Enforces the core invariant — AI-detected allergen data is never shown as reliable until human-confirmed.
...
- **Customer "ask staff" banner:** the order page (`app/trucks/[slug]/order/page.tsx`) shows a prominent amber notice above the menu when `truck.allergensVerified === false` — *"Allergen information has not been verified for this menu. If you have a food allergy or intolerance, please speak to the staff before ordering."* NOT a block (customers can still order, just informed); nothing unreliable sits beside it because the chips are already hidden.
...
**ALLERGEN REVIEW WORKFLOW — SPEC ONLY, NOT BUILT [POST-DEPLOY, SAFETY-CRITICAL]. (full spec in memory `project_allergen_workflow.md`).** Build deliberately AFTER the Gusto deploy.
...
> ⚠️ **DEPLOYMENT STATUS (V8.0 — read before shipping).** Additive on the V7.9 working tree (still uncommitted). **Migrations already applied:** the V7.9 set (`20260624`, `20260625`), `event_option_stock`, and the order-ready columns (`truck_vans.order_ready_enabled`, `truck_events.order_ready_override`). **PENDING-APPLY, deploy-coupled:**
...
**§61 — Per-truck scrape frequency (background infrastructure; NO control UI). [BUILT, node-syntax-clean; 3 migrations PENDING; activates at merge-to-main].** Each HatchGrab-linked truck (a `trucks` row with `scraper_preference IN ('auto','both')` + a `schedule_url`) is re-scraped on its OWN cadence rather than the old single daily pass. The cadence is governed by **three AND-composed gates** evaluated per truck per run in the Pass-B loop (`scripts/run-scraper.js`): a truck scrapes this run IFF `shouldRunToday(truck) && isScrapeSlotNow(truck) && dueByFrequency(truck)` (V8.0 added the fixed local-timezone slots — see below).
...
- **4 PENDING migrations — all ADDITIVE / safe-early (apply by hand anytime BEFORE the deploy):** `ALTER TABLE trucks ADD COLUMN scrape_times_per_day int NOT NULL DEFAULT 3` + `UPDATE trucks SET scrape_times_per_day = 1`; `ADD COLUMN scraper_last_run_at timestamptz`; `ADD COLUMN scraper_last_text_hash text`; `ADD COLUMN timezone text` (V8.0 — null = `Europe/London` via the gate's fallback; set per-truck for non-UK). The first three read as "daily"/"due"/"changed" when null; **`timezone` is also safe-early** because NOTHING in the currently-deployed `main` references it — the new Pass-B `select` (which lists `timezone`) only runs once the branch deploys, by which point the column exists. So apply all four early and they sit inert until the scraper code ships. (Contrast §65's `place_order_atomic` rewrite, which IS deploy-coupled — applying it early would break the live old TS that still passes `p_draw_list`.)
- **DEPLOY-COUPLING:** GitHub Actions schedules run only from the DEFAULT branch, so the new cadence **activates at merge-to-`main`, not before** — land the `run-scraper.js` gating + BOTH workflow YAMLs together (a partial deploy would drop Pass A or double-run Pass B). Until merged, the current `daily_scrape.yml` on `main` keeps running both passes 1×/day. Post-deploy, verify BOTH Actions jobs register + run (discovery 1×/day, hatchgrab every-4h).
