...
Delta over V8.9. Keep-screen-on made **per-device**; the **wake-lock activation properly fixed** (passive banner → real button); and the **`/api/dashboard` hand-picked-subset bug class SWEPT and CLOSED**. `lib/native/keepAwake.ts` + dashboard/KDS + one dead action removed + docs. **Status: BUILT, tsc-clean, UNDEPLOYED (Gusto is live).**
