...
Operator-path + capacity-correctness + offline-protection + event-conflict session. Unified operator/customer capacity inputs (catConfigs DRY); fixed instant-point tone under-colouring and the first-order slot-usage double-count; made the usage rebuild fail loudly; added a durable offline-pause marker + reconnect notification; added two-check event-conflict detection (duplicate + overlap) across scraper and operator-added events. Large set of logged post-trial design items. Most fixes tsc-clean, NOT live-verified.
