...
| **G2** | **The KDS grid caps visible orders at 8, and the hidden ones are unreachable.** `MAX_GRID_VISIBLE = 8`, introduced in WIP commit `1d3d73c` for a fixed-column grid that no longer exists. The panel already scrolls, and the "+N more" indicator is a `<div>`, not a button. | ✅ **CLOSED (V13.1, 14 September 2026) — THE CAP IS GONE.** 🧪 Re-derived: `MAX_GRID_VISIBLE` has **one** occurrence left in the repository and it is a comment reading *"THE GRID CAP IS GONE"* (positive control over the same file: `activeLayout` appears 9 times as live code). It was **not a demo bug** — a busy real service hid orders 9+ with no way to reach them — which is why it was removed rather than demo-gated. |
...
| **G11** | **The KDS's `DemoGetStarted` receives neither `isAdmin` nor `extractionSource`**, because that page makes no `/api/auth/me` and reads no `/api/dashboard` demo block. 🧪 Re-verified V13.1: the KDS renders `<DemoGetStarted token={token} />` bare, and a search for `data.demo\|demoSession\|extraction_source` in that file returns **nothing** (control: the dashboard has 6 `demoSession` references). | 🔴 **STILL PRESENT, AND THE ROW ABOVE USED TO DESCRIBE TWO DIFFERENT STATES AT ONCE — corrected V13.1.** `canSetup = (NEXT_PUBLIC_SIGNUP_PUBLIC === 'true' \|\| isAdmin) && !!token`, and `variant = !canSetup ? 'saveOnly' : extractionSource === 'template' ? 'sample' : 'upload'`. **Today the flag is not `'true'`, so the KDS shows `saveOnly` — "Save my menu" — exactly as the dashboard shows a prospect, and there is no divergence to see.** The defect is **latent**: the moment the flag flips, the KDS shows the **`upload`** copy on a SAMPLE demo, calling a template menu "your menu", which §11 forbids. An admin also gets `saveOnly` here where the dashboard gives the wizard. **Close it in the same change that flips the flag** — see the reference manual's Backlog #17. |
