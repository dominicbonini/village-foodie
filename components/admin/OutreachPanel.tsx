...
type SortKey = 'name' | 'phone' | 'whatsapp' | 'email' | 'hu_map' | 'hu_ordering' | 'schedule' | 'upcoming' | 'stage' | 'last_contacted' | 'next_action'
...
const COLUMNS: { key: SortKey; label: string; title?: string }[] = [
  ...
  { key: 'schedule', label: 'Schedule', title: 'Do we hold a schedule for this truck at all? Y = we know of at least one event, past or future. N = none known. Hover the cell for the last event date.' },
  { key: 'upcoming', label: 'Upcoming', title: 'Number of FUTURE events we hold for this truck (discovery_events dated today or later).' },
...
function sortValue(p: Prospect, key: SortKey, _hatchesUp: (v: string | null) => boolean): number | string | null {
  switch (key) {
    ...
    case 'schedule':
      return (p.futureEventCount > 0 || p.lastEventDate) ? 1 : 0
    case 'upcoming':
      return p.futureEventCount
    ...
    <div className="text-slate-900">
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="overflow-auto rounded-xl border border-slate-200 bg-white max-h-[calc(100vh-9rem)]">
          <table className="table-fixed text-sm w-full" style={{ minWidth: '1510px' }}>
            <colgroup>
              ...
              <col style={{ width: '96px' }} />{/* schedule — Y/N */}
              <col style={{ width: '104px' }} />{/* upcoming — count */}
...
const Row = memo(function Row({ p, onOpen, onPatch }: {
  ...
  const hasSchedule = p.futureEventCount > 0 || !!p.lastEventDate
  const schedTitle = hasSchedule
    ? (p.lastEventDate ? `Last event ${fmtDate(p.lastEventDate)}` : 'Upcoming events only — no past event recorded')
  ...
  return (
    <tr className="border-t border-slate-100">
      ...
      <td className="px-3 py-2 text-center" title={schedTitle}>
        <span className={`font-bold ${hasSchedule ? 'text-slate-700' : 'text-slate-300'}`}>{hasSchedule ? 'Y' : 'N'}</span>
      </td>
      {/* Upcoming: a zero is meaningful (we hold a schedule but nothing is booked ahead), so it is shown
          as 0 and greyed rather than hidden behind an em dash. */}
      <td className="px-3 py-2 text-center tabular-nums">
        <span className={p.futureEventCount > 0 ? 'text-slate-700 font-medium' : 'text-slate-300'}>{p.futureEventCount}</span>
      </td>
