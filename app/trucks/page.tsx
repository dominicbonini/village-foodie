    ...
    <main className="min-h-screen bg-slate-50 flex flex-col items-center py-10 px-4">
      <div className="max-w-2xl w-full">
        ...
        {loading ? (
          ...
          /* REMOVED overflow-hidden so the sticky menu actually works! */
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 relative">
            
            {/* FULL A-Z JUMP MENU */}
            <div className="bg-white/95 backdrop-blur-md border-b border-slate-200 py-3 px-2 sm:px-4 sticky top-0 z-20 flex flex-wrap justify-center gap-0.5 shadow-sm rounded-t-2xl">
              {alphabet.map(letter => {
              ...
              <ul className="flex flex-col p-4 md:p-6">
                {sortedTrucks.map((truck, index) => {
                  ...
                  return (
                    <li 
                      key={truck.cleanKey} 
                      // Added an ample scroll margin to clear the multi-line sticky menu
                      id={isFirstOfLetter ? `letter-${letterKey}` : undefined}
                      className={isFirstOfLetter ? 'scroll-mt-[130px]' : ''}
                    >
                      <Link 
