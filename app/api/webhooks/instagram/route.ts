...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('[webhook/instagram] REFUSED - could not read request body:', err)
  ...
  if (!verification.ok) {
    console.error(metaRefusalLog('instagram', verification.reason, secrets.length, !!signatureHeader, rawBody.length))
  ...
  try {
    ...
    try {
      ...
      console.error('[webhook/instagram] verified but body is not JSON')
      return NextResponse.json({ ok: true })
    }
    const parsed = body as any
    console.log('[webhook/instagram] incoming:', JSON.stringify(parsed, null, 2))
